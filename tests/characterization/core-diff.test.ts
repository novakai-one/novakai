...
test('diffModels: an added node is reported and does not affect unchanged counts', () => {
  const before = { nodes: { a: mkN('a', 'A'), b: mkN('b', 'B') }, edges: [mkE('e1', 'a', 'b')] };
  const after = { nodes: { a: mkN('a', 'A'), b: mkN('b', 'B'), c: mkN('c', 'C') }, edges: [mkE('e1', 'a', 'b')] };
  const d = diffModels(before, after);
  assert.deepEqual(d.addedNodes, ['c']);
  assert.deepEqual(d.counts, { nAdd: 1, nRem: 0, nChg: 0, eAdd: 0, eRem: 0, nUnchanged: 2, eUnchanged: 1 });
});
...
test('diffModels: a removed node also drops its incident edge as removed', () => {
  ...
  const d = diffModels(before, after);
  assert.deepEqual(d.removedNodes, ['b']);
...
test('diffModels: identical frontmatter -> no change reported', () => {
  const fm = { name: 'X', description: 'd', state: ['s1'], interfaces: [{ name: 'i', accepts: ['a'], returns: ['b'] }] };
  const before = { nodes: { a: mkN('a', 'A', { fm }) }, edges: [] };
  const after = { nodes: { a: mkN('a', 'A', { fm: { ...fm } }) }, edges: [] };
...
test('diffModels: a changed frontmatter description reports field "fm" with §-joined signatures', () => {
  const fm = { name: 'X', description: 'd', state: ['s1'], interfaces: [{ name: 'i', accepts: ['a'], returns: ['b'] }] };
  const before = { nodes: { a: mkN('a', 'A', { fm }) }, edges: [] };
  const after = { nodes: { a: mkN('a', 'A', { fm: { ...fm, description: 'd2' } }) }, edges: [] };
  assert.deepEqual(diffModels(before, after).changedNodes, [{
