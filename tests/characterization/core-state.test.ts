...
function mkNode(
  ...
  opts: { width?: number; height?: number; extra?: any } = {},
): any {
  const { width = 160, height = 56, extra = {} } = opts;
  return { id, label: id, shape: 'rect', color: null, x, y, 'w': width, 'h': height, ...extra };
}
...
test('portPos: pt/pb/pl/pr on a 100x50 node at (10,20)', () => {
  const node = mkNode('n1', 10, 20, { width: 100, height: 50 });
  assert.deepEqual(portPos(node, 'pt'), { x: 60, y: 20 });
  assert.deepEqual(portPos(node, 'pb'), { x: 60, y: 70 });
  assert.deepEqual(portPos(node, 'pl'), { x: 10, y: 45 });
  assert.deepEqual(portPos(node, 'pr'), { x: 110, y: 45 });
});

test('nodeCenter: centre of a 100x50 node at (10,20)', () => {
  assert.deepEqual(nodeCenter(mkNode('n1', 10, 20, { width: 100, height: 50 })), { 'cx': 60, 'cy': 45 });
});
...
test('bestSides: a left of b (dx dominant, positive) -> [pr, pl]', () => {
  assert.deepEqual(
    bestSides(mkNode('a', 0, 0, { width: 100, height: 50 }), mkNode('b', 300, 0, { width: 100, height: 50 })),
    ['pr', 'pl'],
  );
});
...
test('bestSides: a right of b (dx dominant, negative) -> [pl, pr]', () => {
  assert.deepEqual(
    bestSides(mkNode('a', 300, 0, { width: 100, height: 50 }), mkNode('b', 0, 0, { width: 100, height: 50 })),
    ['pl', 'pr'],
  );
});
...
test('bestSides: a above b (dy dominant, positive) -> [pb, pt]', () => {
  assert.deepEqual(
    bestSides(mkNode('a', 0, 0, { width: 100, height: 50 }), mkNode('b', 0, 300, { width: 100, height: 50 })),
    ['pb', 'pt'],
  );
});
...
test('bestSides: a below b (dy dominant, negative) -> [pt, pb]', () => {
  assert.deepEqual(
    bestSides(mkNode('a', 0, 300, { width: 100, height: 50 }), mkNode('b', 0, 0, { width: 100, height: 50 })),
    ['pt', 'pb'],
  );
});
...
test('bestSides: identical centres (dx=dy=0) -> falls to the vertical branch [pt, pb]', () => {
  assert.deepEqual(
    bestSides(mkNode('a', 0, 0, { width: 100, height: 50 }), mkNode('a', 0, 0, { width: 100, height: 50 })),
    ['pt', 'pb'],
  );
});
...
test('nodeFootprint: no measured card -> footprint is just the node box', () => {
  ...
  state.nodes['n1'] = mkNode('n1', 10, 20, { width: 100, height: 50 });
  assert.deepEqual(nodeFootprint(state, state.nodes['n1'], true), { x: 10, y: 20, 'w': 100, 'h': 50 });
});

test('nodeFootprint: measured card + showFrontmatter=true widens/heightens and re-centres x', () => {
  const state = createState();
  state.nodes['n1'] = mkNode('n1', 10, 20, { width: 100, height: 50 });
  state.measured.set('n1', { cardW: 200, cardH: 30 });
  assert.deepEqual(nodeFootprint(state, state.nodes['n1'], true), { x: -40, y: 20, 'w': 200, 'h': 86 });
});
...
test('nodeFootprint: measured card present but showFrontmatter=false -> still just the box', () => {
  ...
  state.nodes['n1'] = mkNode('n1', 10, 20, { width: 100, height: 50 });
  state.measured.set('n1', { cardW: 200, cardH: 30 });
  assert.deepEqual(nodeFootprint(state, state.nodes['n1'], false), { x: 10, y: 20, 'w': 100, 'h': 50 });
});
...
test('frameTransform: wantZ within [zMin,zMax] -> used as-is, centres the node', () => {
  assert.deepEqual(
    frameTransform(mkNode('n1', 100, 100, { width: 200, height: 100 }), 800, 600, 1, 0.2, 3),
    { x: 200, y: 150, 'z': 1 },
  );
});
...
test('frameTransform: wantZ above zMax -> clamped to zMax', () => {
  assert.deepEqual(
    frameTransform(mkNode('n1', 100, 100, { width: 200, height: 100 }), 800, 600, 5, 0.2, 3),
    { x: -200, y: -150, 'z': 3 },
  );
});
...
test('frameTransform: wantZ below zMin -> clamped to zMin', () => {
  assert.deepEqual(
    frameTransform(mkNode('n1', 100, 100, { width: 200, height: 100 }), 800, 600, 0.05, 0.2, 3),
    { x: 360, y: 270, 'z': 0.2 },
  );
});
...
test('worldBounds: bounding box across all nodes', () => {
  const state = createState();
  state.nodes['a'] = mkNode('a', 0, 0, { width: 100, height: 50 });
  state.nodes['b'] = mkNode('b', 200, 150, { width: 100, height: 50 });
  assert.deepEqual(worldBounds(state), { minX: 0, minY: 0, maxX: 300, maxY: 200 });
});

test('levelBounds: container with no children -> null', () => {
  const state = createState();
  state.nodes['a'] = mkNode('a', 0, 0, { width: 100, height: 50 });
  assert.equal(levelBounds(state, 'nope'), null);
});
...
test('levelBounds: top level (null container) -> bounding box of top-level nodes', () => {
  ...
  state.nodes['a'] = mkNode('a', 0, 0, { width: 100, height: 50 });
  state.nodes['b'] = mkNode('b', 200, 150, { width: 100, height: 50 });
  assert.deepEqual(levelBounds(state, null), { minX: 0, minY: 0, maxX: 300, maxY: 200 });
});
...
function containmentState(): any {
  const state = createState();
  state.nodes['parent'] = mkNode('parent', 0, 0, { width: 300, height: 300 });
  state.nodes['g1'] = mkNode('g1', 10, 10, {
    width: 280, height: 280, extra: { shape: 'group', parent: 'parent' },
  });
  state.nodes['n1'] = mkNode('n1', 20, 20, { width: 100, height: 50, extra: { parent: 'g1' } });
  state.nodes['n2'] = mkNode('n2', 500, 500, { width: 100, height: 50 });
  return state;
}

test('containerOf: a node whose parent is a group reports the group\'s own (non-group) container', () => {
  const state = containmentState();
  assert.equal(containerOf(state, 'n1'), 'parent');
});

test('containerOf: a group with a non-group parent reports that parent', () => {
  const state = containmentState();
  assert.equal(containerOf(state, 'g1'), 'parent');
});

test('containerOf: top-level nodes (rect or group parent) report null', () => {
  const state = containmentState();
  assert.equal(containerOf(state, 'parent'), null);
  assert.equal(containerOf(state, 'n2'), null);
});

test('childIdsOf: "parent" level includes the group and the node inside it (group is transparent)', () => {
  const state = containmentState();
  assert.deepEqual(childIdsOf(state, 'parent').sort(), ['g1', 'n1']);
});

test('childIdsOf: top level includes "parent" and "n2", not the nested nodes', () => {
  const state = containmentState();
  assert.deepEqual(childIdsOf(state, null).sort(), ['n2', 'parent']);
});
...
test('nodeAtPoint: hits a leaf node at the given drill level', () => {
  const state = containmentState();
  assert.equal(nodeAtPoint(state, 30, 30, 'parent'), 'n1');
});

test('nodeAtPoint: empty spot inside the group box -> falls back to the group (low priority)', () => {
  const state = containmentState();
  assert.equal(nodeAtPoint(state, 250, 250, 'parent'), 'g1');
});

test('nodeAtPoint: no node under the point -> null', () => {
  const state = containmentState();
  assert.equal(nodeAtPoint(state, 999, 999, 'parent'), null);
});

test('nodeAtPoint: top level, hits a plain top-level node', () => {
  const state = containmentState();
  assert.equal(nodeAtPoint(state, 520, 520, null), 'n2');
});
...
test('sliceIds: solid edges pulled transitively downstream (a->b->c) and upstream, plus 1-hop dotted', () => {
  const state = createState();
  state.nodes['a'] = mkNode('a', 0, 0);
  state.nodes['b'] = mkNode('b', 0, 0);
  state.nodes['c'] = mkNode('c', 0, 0);
  state.nodes['d'] = mkNode('d', 0, 0);
  state.edges = [
  ...
  assert.deepEqual([...sliceIds(state, 'a')].sort(), ['a', 'b', 'c', 'd']);
  assert.deepEqual([...sliceIds(state, 'c')].sort(), ['a', 'b', 'c']);
});
