...
test('parseGroupDirective: %% group line sets hier.groups, returns true', () => {
  ...
  const matched = parseGroupDirective('%% group g1 "Group Label"', hier);
...
test('parseGroupDirective: %% group line with parent captures parent id', () => {
  const hier = { groups: {}, memberOf: {} } as any;
  const matched = parseGroupDirective('%% group g2 "Sub" parent g1', hier);
  assert.equal(matched, true);
...
test('parseGroupDirective: %% group-member line sets hier.memberOf, returns true', () => {
  const hier = { groups: {}, memberOf: {} } as any;
  const matched = parseGroupDirective('%% group-member g1 n1', hier);
  assert.equal(matched, true);
...
test('parseGroupDirective: non-matching line returns false, hier untouched', () => {
  const hier = { groups: {}, memberOf: {} } as any;
  const matched = parseGroupDirective('some other line', hier);
...
test('fromMermaid: empty text yields empty model with defaults', () => {
  const state = fromMermaid('');
  assert.deepEqual(state, {
    nodes: {}, edges: [], nextN: 1, nextE: 1, dir: 'TD', roots: [], hier: { groups: {}, memberOf: {} },
  });
});
...
n1 --> n2
`;
  const state = fromMermaid(basic);
  assert.equal(state.dir, 'LR');
  assert.equal(state.nextN, 3);
  assert.equal(state.nextE, 2);
  assert.deepEqual(state.nodes.n1, {
    id: 'n1', label: 'Hello', shape: 'rect', color: null, x: 80, y: 80, 'w': 160, 'h': 56,
  });
  assert.deepEqual(state.nodes.n2, {
    id: 'n2', label: 'World', shape: 'round', color: null, x: 280, y: 80, 'w': 160, 'h': 56,
  });
  assert.deepEqual(state.edges, [{ id: 'e1', from: 'n1', 'to': 'n2', label: '', style: 'solid', routing: 'straight' }]);
});

const fullMetadataMmd = `flowchart TB
...
test('fromMermaid: full metadata (fm, kind, parent, root, edge ortho/bend/labelpos, group)', () => {
  const state = fromMermaid(fullMetadataMmd);
  assert.equal(state.dir, 'TD'); // TB is normalized to TD
  assert.deepEqual(state.nodes.n1, {
    id: 'n1', label: 'Label A', shape: 'rect', color: null, x: 10, y: 20, 'w': 160, 'h': 56, kind: 'component',
  });
  assert.deepEqual(state.nodes.n2, {
    id: 'n2', label: 'Label B', shape: 'rect', color: null, x: 80, y: 80, 'w': 160, 'h': 56, parent: 'n1',
  });
  assert.deepEqual(state.edges, [{
    id: 'e1', from: 'n1', 'to': 'n2', label: 'edge label', style: 'solid', routing: 'ortho',
    bend: { x: 5, y: 6 }, labelPos: { x: 7, y: 8 },
  }]);
  assert.deepEqual(state.roots, ['n1']);
  assert.deepEqual(state.hier, {
    groups: { 'g1': { id: 'g1', label: 'Group One', parent: null } }, memberOf: { 'n1': 'g1' },
  });
});

test('fromMermaid: subgraph block sets group shape + child parent, "end" pops stack', () => {
  const subgraphMmd = `flowchart TD
subgraph g1 ["My Group"]
  n1["Inside"]
end
n2["Outside"]
`;
  const state = fromMermaid(subgraphMmd);
  assert.equal(state.nodes.g1.shape, 'group');
  assert.equal(state.nodes.n1.parent, 'g1');
  assert.equal(state.nodes.n2.parent, undefined);
});
...
n1["A"]
`;
  const state = fromMermaid(dangling);
  // parent "ghost" was never declared -> nulled
  assert.equal(state.hier.groups.g1.parent, null);
  // group "ghost2" was never declared -> membership dropped
  assert.deepEqual(state.hier.memberOf, {});
});
...
e["E"]
`;
  const state = fromMermaid(auto);
  assert.equal(state.nodes.a.x, 80);
  assert.equal(state.nodes.a.y, 80);
  assert.equal(state.nodes.d.x, 680);
  assert.equal(state.nodes.d.y, 80);
  assert.equal(state.nodes.e.x, 80);
  assert.equal(state.nodes.e.y, 210);
});
...
const state1 = {
  ...
  nodes: {
    ...
    'n2': {
      id: 'n2', label: 'Node Two', shape: 'round', color: '#262c4a', x: 200, y: 20, 'w': 160, 'h': 56, parent: 'n1',
    },
  },
...
test('initMermaid.applyText: parses text, replaces state, fires render/pushHistory/toast', () => {
  const { ctx, calls } = makeCtx({
    nodes: {}, edges: {}, nid: 1, eid: 1, dir: 'TD', roots: [], hier: { groups: {}, memberOf: {} },
  });
  ctx.dom.mmd.value = 'flowchart TD\nn1["Hi"]\n';
  const api = initMermaid(ctx, selection);
  api.applyText();
  assert.deepEqual(ctx.state.nodes.n1, {
    id: 'n1', label: 'Hi', shape: 'rect', color: null, x: 80, y: 80, 'w': 160, 'h': 56,
  });
