/* =====================================================================
   contracts.ts — K4 Contracts tab stub (K-seam pre-wiring)
   ---------------------------------------------------------------------
   Responsibility: initContracts(ctx) follows the house initX(ctx) shape
   so K4 can grow this into the real keystone certificate view without
...
const TAB_ID = 'contracts';

export function initContracts(ctx: AppContext): ContractsApi {
  void ctx; // no state at this stub stage — kept for the house initX(ctx) shape K4 will fill in
