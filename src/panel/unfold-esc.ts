/* unfold-esc.ts — the pure Esc-chain decision for the primary surface
   ...
   the acceptance runner). Priority order focusType > selWire > stage > sel
   > query, and 'none' at the bottom — Escape never exits unfold (the old
   close() branch is gone by design). The keydown handler in unfold.ts is a
   thin applier of this result. */

export function ufEscAction(s: { focusType: boolean; selWire: boolean; stage: boolean; sel: boolean; query: boolean }):
  'clearTypeFocus' | 'deselectWire' | 'exitStage' | 'selectGroup' | 'clearQuery' | 'none' {
