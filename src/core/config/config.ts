 ...
 */
export function nodeFill(node: { color: string | null; kind?: NodeKind | null }): string | null {
  return node.color ?? (node.kind ? KIND_TINT[node.kind] : null);
}
...
// quoted: w/h are ShapeDefault's frozen field names; id-length would flag bare keys
...
// quoted: pt/pb/pl/pr are external port-anchor ids; id-length would flag bare keys
...
export const THEMES: Record<string, Theme> = {
  slate: { name: 'Slate', vars: {
    ...
    '--line': '#2a3140', '--line-bright': '#3a4254', '--ink': '#e7eaf1',
    '--ink-dim': '#8b94a6', '--ink-faint': '#565f6e',
    '--accent': '#7c8cff', '--accent-2': '#d9a066', '--accent-3': '#6f9bd8', '--danger': '#e9697f', '--sel': '#7c8cff',
    '--node-bg': '#1d222c', '--node-stroke': '#3a4254', '--node-ink': '#e7eaf1',
    '--note-bg': '#292318', '--note-stroke': '#4a3f2a', '--note-ink': '#ecdcb8',
...
export const FONTS: Record<string, FontDef> = {
  ...
  rounded: {
    name: 'Rounded',
    stack: "ui-rounded, 'SF Pro Rounded', 'Hiragino Maru Gothic ProN', 'Varela Round', system-ui, sans-serif",
  },
...
export function esc(str: string | null | undefined): string {
  return (str || '').replace(/[&<>]/g, (char) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[char] as string));
}

/** Escape a label for embedding inside Mermaid double-quoted text. */
export function escM(str: string | null | undefined): string {
