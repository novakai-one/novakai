...
const LEVEL_POSITIONS_SRC_DIRECTIVE = '%% src levelPositions src/core/plan/plan.ts#levelPositions\n';
const PLAN_TS_PATH = 'src/core/plan/plan.ts';
const PROJECT_A_X = '(result) => result.a.x';

test('srcDirectives + collectCases resolve a change to a real export', () => {
  const map = srcDirectives(LEVEL_POSITIONS_SRC_DIRECTIVE);
  assert.deepEqual(map.levelPositions, { path: PLAN_TS_PATH, symbol: 'levelPositions' });
  ...
  assert.equal(cases[0].path, PLAN_TS_PATH);
});
...
const AVAILABLE = PRE.ran && PRE.results[0]
  && (PRE.results[0].pass || !/runner failed/.test(PRE.results[0].error || ''));
...
test('a CORRECT behavioural case against real levelPositions goes GREEN', { skip: !AVAILABLE }, () => {
  ...
  assert.ok(res.ran);
  assert.equal(res.results.every((result) => result.pass), true,
    `expected all green, got ${JSON.stringify(res.results)}`);
});
...
test('a WRONG expectation goes RED (the gap the signature gate cannot catch)', { skip: !AVAILABLE }, () => {
  ...
  const res = runAcceptance({ planPath, mapPath: BUNDLE });
  assert.equal(res.results.some((result) => !result.pass), true, 'a wrong expectation must fail');
});
...
test('a symbol with no map mapping (unimplemented) is RED', { skip: !AVAILABLE }, () => {
  ...
  const res = runAcceptance({ planPath, mapPath: BUNDLE });
  assert.equal(res.results.every((result) => !result.pass), true, 'an unmapped/unimplemented symbol must be red');
});

/** Write the lens-escape-hatch fixture plan: its acc.path/acc.symbol must
 *  win over the (deliberately broken) map's own `%% src` directive. */
function writeLensEscapeHatchPlan(planPath) {
  writeFileSync(planPath, JSON.stringify({
    base: 'test',
    changes: [{ id: 'c1', status: 'modify', target: { kind: 'node', ref: 'levelPositions' },
      intent: { problem: '', approach: '' },
      acceptance: {
        path: PLAN_TS_PATH, symbol: 'levelPositions',
...
test('acc.path/acc.symbol wins over a wrong `%% src` mapping (pure lens escape hatch)', { skip: !AVAILABLE }, () => {
  ...
  writeLensEscapeHatchPlan(planPath);
  const res = runAcceptance({ planPath, mapPath });
  assert.ok(res.ran);
  assert.equal(res.results.every((result) => result.pass), true,
    `expected green via acc.path lens, got ${JSON.stringify(res.results)}`);
});
...
test('collectCases lifts kind:projection + the lens onto the case', () => {
  ...
  const caseResult = collectCases(plan, map)[0];
  assert.equal(caseResult.kind, 'projection');
  assert.equal(caseResult.projection, '(r) => r.a');
});
...
test('collectCases defaults a case to kind:pure with no lens', () => {
  ...
  const caseResult = collectCases(plan, map)[0];
  assert.equal(caseResult.kind, 'pure');
  assert.equal(caseResult.projection, null);
});
...
test('a projection case asserting a SLICE of the real result goes GREEN', { skip: !AVAILABLE }, () => {
  const planPath = writePlan([
    { name: 'project the x of node a', kind: 'projection',
      args: [[{ id: 'a', x: 11, y: 22, synth: false }]],
      projection: PROJECT_A_X, equals: 11 },
  ]);
  const res = runAcceptance({ planPath, mapPath: BUNDLE });
  assert.ok(res.ran);
  assert.equal(res.results.every((result) => result.pass), true, `expected green, got ${JSON.stringify(res.results)}`);
});
...
test('a projection case with a WRONG slice expectation goes RED', { skip: !AVAILABLE }, () => {
  const planPath = writePlan([
    { name: 'wrong projected slice', kind: 'projection',
      args: [[{ id: 'a', x: 11, y: 22, synth: false }]],
      projection: PROJECT_A_X, equals: 999 },
  ]);
  const res = runAcceptance({ planPath, mapPath: BUNDLE });
  assert.equal(res.results.some((result) => !result.pass), true, 'a wrong projected slice must fail');
});
...
test('a projection case with NO lens is RED (not a silent pass)', { skip: !AVAILABLE }, () => {
  ...
  const res = runAcceptance({ planPath, mapPath: BUNDLE });
  assert.equal(res.results.every((result) => !result.pass), true, 'a projection case with no lens must be red');
});
...
test('a projection run is byte-identical across two runs (replay idiom)', { skip: !AVAILABLE }, () => {
  const cases = [{ name: 'project x', kind: 'projection',
    args: [[{ id: 'a', x: 11, y: 22, synth: false }]],
    projection: PROJECT_A_X, equals: 11 }];
  const run1 = runAcceptance({ planPath: writePlan(cases), mapPath: BUNDLE });
  const run2 = runAcceptance({ planPath: writePlan(cases), mapPath: BUNDLE });
  assert.equal(JSON.stringify(run1.results), JSON.stringify(run2.results),
    'projection results must be identical across runs');
});
