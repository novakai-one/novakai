# Building a Novakai (procedure for an AI or a human)

This is the entry point. Read it before building a novakai of any repo, regardless of
how familiar you are with Novakai. It exists because the common failure is producing a
**flat file-mirror** (one node per file, no sections, dotted wiring) that is grammar-valid
but useless for review. `novakai-validate` does NOT catch that; `novakai-lint` does.

## Definition of done (non-negotiable)

```
novakai-lint <bundle>.mmd   # must exit 0, with NO FLAT and NO LOOSE-BAG errors
```

Done is not "it renders" or "validate passes". Done is **lint exit 0**. The `novakai:ship`
and `novakai:verify` scripts enforce this: they fail unless validate AND lint pass.
...
2. **The draft FAILS `novakai-lint` by design** (it is a file-mirror: one node per
...
4. After authoring your fragments (steps 1–7), run **`npm run novakai:backfill`**
...
1. **If a novakai already exists, lint it FIRST.** If it fails, distrust it — do not imitate
   it, rebuild. A failing existing novakai is the trap that produces another bad one.
...
- **When the repo is code-extractable, run the gate** (`novakai-extract` + `novakai-gate`) to
  cross-check the diagram against the real TypeScript — that is the only check that compares the
  map to reality.

## Worked examples (in tools/novakai/verify/fixtures/)
...
- `loop-demo-v1-loose.mmd` — the first-draft mistake. `novakai-lint` it: FAIL (LOOSE-BAG).
- `loop-demo-v2-fixed.mmd` — the same module fixed. `novakai-lint` it: PASS.
...
1. Point a fresh LLM at this repo. Tell it to build a novakai of some target repo, ending at
   `novakai-lint <bundle> ` exit 0.
2. Run `novakai-lint --report` on its output yourself.
