# novakai-spec-tools — how it's distributed and consumed (read me before "packaging" work)

`tools/` is a self-contained npm package: **`novakai-spec-tools`**. It is NOT the app
(the app is the repo root, a Vite project — a different `package.json`). The package ships
seven CLI bins — `novakai-bundle`, `novakai-validate`, `novakai-lint`, `novakai-extract`,
`novakai-gate`, `novakai-stubs`, `novakai-scaffold` — and declares `ts-morph` as a real
...
NovaKai depends on it locally:
    ...
    "devDependencies": { "novakai-spec-tools": "file:../../novakai/tools" }
...
The package is already publish-ready (the name `novakai-spec-tools` is free on the registry;
...
Then point each consumer at the registry instead of the path:
    ...
    cd <consumer>
    npm pkg set devDependencies.novakai-spec-tools="^0.1.0"
    npm install && npm run novakai:verify            # confirm the registry-installed bin passes
    git commit -am "chore: consume novakai-spec-tools from the registry"

Private/org instead: scope the name to `@novakai-one/novakai-spec-tools`, add `.npmrc` with
...
A consumer populates the inspector's source pane via `%% src` directives in their `.novakai.mmd` fragments +
`novakai-extract --map` (shipped in this package). Both this repo and consumers use the same
