...
if (CONTINUE) {
  ...
  const contQuiz = run('node tools/flowmap/onboard/quiz.mjs verify' + SESSION_ARG);
  ...
  line('');
  const contFresh = run('node tools/flowmap/status/handoff-fresh.mjs --check');
  if (contFresh.ok) {
...
const quizState = run('node tools/flowmap/onboard/quiz.mjs verify' + SESSION_ARG);
...
const fresh = run('node tools/flowmap/status/handoff-fresh.mjs --check');
if (fresh.ok) {
  line('✓ HANDOFF TRUSTWORTHY — no claim in docs/flowmap/SESSION_HANDOFF.md is falsified by the committed tree.\n');
} else {
