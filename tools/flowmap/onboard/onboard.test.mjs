...
const r = spawnSync('node', [join('tools', 'flowmap', 'onboard', 'onboard.mjs')],
...
test('F-17 deny: onboard exits 1 on a doctored checkout (map incomplete vs code)', () => {
  ...
  try {
    ...
    rmSync(frag);
    const deny = spawnSync('node', [join('tools', 'flowmap', 'onboard', 'onboard.mjs')],
...
test('continue track: scoped pointers, scoped quiz commands, and the verbatim out-of-scope rule', () => {
  const c = spawnSync('node', [join('tools', 'flowmap', 'onboard', 'onboard.mjs'), '--continue',
