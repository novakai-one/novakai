/* =====================================================================
   ...
   The REAL corpus (tools/flowmap/verify/mutations.json) is deliberately NOT run
...
function runMutate(dir, corpusObj) {
  ...
  return spawnSync('node', [join('tools', 'flowmap', 'verify', 'mutate.mjs'),
...
test('harness: the REAL corpus is not stale (every find-string matches HEAD exactly once)', () => {
  ...
  const corpus = JSON.parse(readFileSync(join(ROOT, 'tools', 'flowmap', 'verify', 'mutations.json'), 'utf8'));
