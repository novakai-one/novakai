...
const MAP = join(ROOT, 'docs/flowmap/_bundle.mmd');
const ALLOW = join(ROOT, 'docs/flowmap/edge-advisory-allowlist.txt');
...
test('fail-closed CLI: --strict exits 1 when an edge is unaccounted', () => {
  ...
  const r = spawnSync('node', ['tools/flowmap/verify/edge-verify.mjs', '--strict', '--allow', '/no/such/file'],
...
test('green CLI: --strict exits 0 with the real allowlist', () => {
  const r = spawnSync('node', ['tools/flowmap/verify/edge-verify.mjs', '--strict'], { cwd: ROOT, encoding: 'utf8' });
