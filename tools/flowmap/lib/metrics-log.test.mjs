/* metrics-log.test.mjs — M2b: the emitter's three contract points.
   ...
   (3) FLOWMAP_ROOT seam: the hermetic-fixture pattern every gate test uses
...
test('FLOWMAP_ROOT seam: with no explicit root the emitter honors the env var', () => {
  const dir = mkdtempSync(join(tmpdir(), 'metrics-log-'));
  const prev = process.env.FLOWMAP_ROOT;
  try {
    process.env.FLOWMAP_ROOT = dir;
    ...
    if (prev === undefined) delete process.env.FLOWMAP_ROOT; else process.env.FLOWMAP_ROOT = prev;
