...
test('plan-check.mjs CLI: incoherent plan → exit 1; unreadable plan → exit 2', () => {
  ...
  try {
    ...
    const r = cli('tools/flowmap/plan-check.mjs', ['--plan', bad]);
    assert.equal(r.status, 1, `incoherent plan must exit 1:\n${r.stdout}${r.stderr}`);
    const gone = cli('tools/flowmap/plan-check.mjs', ['--plan', join(dir, 'ghost.json')]);
...
test('plan-check.mjs CLI: the real in-flight plan → exit 0 (good-path wiring)', () => {
  const r = cli('tools/flowmap/plan-check.mjs', ['--plan', 'public/plan.json']);
...
test('plan-cert.mjs CLI: no --plan → exit 2; unreadable plan → exit 2', () => {
  const dir = mkdtempSync(join(tmpdir(), 'cli-wiring-'));
  try {
    assert.equal(cli('tools/flowmap/plan-cert.mjs', []).status, 2, 'no --plan is a usage error (2)');
    const gone = cli('tools/flowmap/plan-cert.mjs', ['--plan', join(dir, 'ghost.json')]);
    assert.equal(gone.status, 2, 'unreadable plan is exit 2');
  } finally { rmSync(dir, { recursive: true, force: true }); }
});
...
test('flowmap-lint.mjs CLI: flat file-mirror → exit 1; no path → exit 2; real map → exit 0', () => {
  ...
  try {
    ...
    const r = cli('tools/flowmap/flowmap-lint.mjs', [flat]);
    assert.equal(r.status, 1, `flat mirror must exit 1:\n${r.stdout}${r.stderr}`);
    assert.equal(cli('tools/flowmap/flowmap-lint.mjs', []).status, 2, 'no path is a usage error (2)');
    const real = cli('tools/flowmap/flowmap-lint.mjs', ['docs/flowmap/_bundle.mmd']);
