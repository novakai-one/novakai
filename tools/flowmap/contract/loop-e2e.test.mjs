...
/* =====================================================================
   ...
   (docs/flowmap/plans/m9-loop.plan.json), so the worktree checkout
   ...
   local id — see docs/flowmap/plans/m9-loop.plan.json's own "note" field
   ...
   `npm run flowmap:loop` is itself the per-step audit record. Each step
...
describe('M9 — end-to-end FAIL->PASS loop (docs/flowmap/plans/m9-design.md)', () => {
  let wt;
  let work;
  ...
  before(() => {
    ...
    try {
      ...
      const validate0 = add.status === 0
        ? m9NodeAt(wt, ['tools/flowmap/verify/validate.mjs', 'docs/flowmap/_bundle.mmd']) : { status: 2, stdout: '', stderr: 'worktree not provisioned' };
      ...
      const quiz2 = m9NodeAt(wt, ['tools/flowmap/onboard/quiz.mjs', 'verify']);
      r.step2 = quiz2;
      ...
      const check4 = m9NodeAt(wt, ['tools/flowmap/plan/plan-check.mjs', '--plan', M9_PLAN]);
      r.step4 = check4;
      ...
      const cert5 = m9NodeAt(wt, ['tools/flowmap/plan/plan-cert.mjs', '--plan', M9_PLAN]);
      r.step5 = cert5;
      ...
      const redExport = m9NodeAt(wt, ['tools/flowmap/plan/approve-export.mjs', '--plan', noVerdictsPath, '--out', redExportDir, '--accepted-only']);
      ...
      const exp8 = m9NodeAt(wt, ['tools/flowmap/plan/approve-export.mjs', '--plan', M9_PLAN, '--out', exportDir, '--accepted-only']);
      ...
      const status9 = m9NodeAt(wt, ['tools/flowmap/status/status.mjs', '--plan', exportPlanPath, '--json']);
      r.step9 = status9;
      ...
      const waves10 = m9NodeAt(wt, ['tools/flowmap/contract/waves.mjs', '--plan', exportPlanPath, '--json']);
      r.step10waves = waves10;
      const waves10Body = m9Json(waves10);
      const contract10 = m9NodeAt(wt, ['tools/flowmap/contract/contract.mjs', '--change', M9_CHANGE, '--plan', exportPlanPath, '--json']);
      ...
      const verify11 = m9NodeAt(wt, ['tools/flowmap/contract/verify-change.mjs', '--change', M9_CHANGE, '--plan', exportPlanPath, '--json']);
      r.step11 = verify11;
      ...
      const orch16red = m9NodeAt(wt, ['tools/flowmap/contract/orchestrate.mjs', '--plan', exportPlanPath, '--strict', '--no-worktree', '--json']);
      ...
      const bundle14 = m9NodeAt(wt, ['tools/flowmap/verify/bundle.mjs', '--root', 'docs/flowmap/root.mmd', '--dir', 'src']);
      const bundleHasProbe14 = bundle14.status === 0 && bundle14.stdout.includes(M9_REF);
      if (bundle14.status === 0) writeFileSync(join(wt, 'docs', 'flowmap', '_bundle.mmd'), bundle14.stdout);
      const validate14 = m9NodeAt(wt, ['tools/flowmap/verify/validate.mjs', 'docs/flowmap/_bundle.mmd']);
      const extractedPath14 = join(work, 'm9-extracted.mmd');
      const extract14 = m9NodeAt(wt, ['tools/buildspec/pipeline/extract.mjs', '--map', 'docs/flowmap/_bundle.mmd', '--tsconfig', 'tsconfig.json', '--out', extractedPath14]);
      const gate14 = m9NodeAt(wt, ['tools/buildspec/pipeline/gate.mjs', '--spec', 'docs/flowmap/_bundle.mmd', '--code', extractedPath14]);
      ...
      const verify15 = m9NodeAt(wt, ['tools/flowmap/contract/verify-change.mjs', '--change', M9_CHANGE, '--plan', exportPlanPath, '--strict', '--json']);
      ...
      const orch16green = m9NodeAt(wt, ['tools/flowmap/contract/orchestrate.mjs', '--plan', exportPlanPath, '--strict', '--no-worktree', '--json']);
      r.step16green = orch16green;
      const orch16greenBody = m9Json(orch16green);
      const waves16green = m9NodeAt(wt, ['tools/flowmap/contract/waves.mjs', '--plan', exportPlanPath, '--json']);
      ...
      const replay17 = m9NodeAt(wt, ['tools/flowmap/contract/replay.mjs', '--task', replayTask, '--n', '5', '--json']);
      r.step17 = replay17;
