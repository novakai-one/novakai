...
/* =====================================================================
   ...
   edge ACCOUNTED FOR. Every spec edge is one of:
     ...
     ADVISORY         — neither of the above, but the edge is listed in the
                        audited allowlist docs/flowmap/edge-advisory-allowlist.txt
   ...
   Usage:
     node edge-verify.mjs [--map docs/flowmap/_bundle.mmd] [--tsconfig tsconfig.json]
                          [--allow docs/flowmap/edge-advisory-allowlist.txt]
...
/* ---------------- CLI ---------------- */
function main() {
  const mapPath = resolve(ROOT, arg('--map', 'docs/flowmap/_bundle.mmd'));
  const tsconfig = arg('--tsconfig', 'tsconfig.json');
  const allowPath = resolve(ROOT, arg('--allow', 'docs/flowmap/edge-advisory-allowlist.txt'));
  ...
  if (writeAllow) {
    ...
    const lines = [
      ...
      '# same as docs/flowmap/curation-allowlist.txt for symbols. Remove a line',
  ...
  console.log('=== flowmap edge verification (A5) — every edge code-backed or audited ===\n');
