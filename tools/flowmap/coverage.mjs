...
/* =====================================================================
   ...
   Usage:
     node coverage.mjs [--src src] [--root docs/flowmap/root.mmd]
...
const ROOT = arg('--root', 'docs/flowmap/root.mmd');
...
const fragments = [];
walk(SRC, (p) => { if (p.endsWith('.flowmap.mmd')) fragments.push(p); });
fragments.push(ROOT);
...
const hasSiblingFragment = (p) => {
  try { return !!statSync(p.replace(/\.ts$/, '.flowmap.mmd')); }
  catch { return false; }
};
...
const uncovered = sources
  ...
  .sort();
if (uncovered.length) {
  console.log(`flowmap-coverage: ${uncovered.length} source file(s) not represented in the map:`);
  for (const p of uncovered) console.log('  ✗ ' + relative('.', p));
  console.log('\nEvery module must be documented. Add a `*.flowmap.mmd` fragment beside the file');
  console.log('(or fold its symbols into a parent fragment via `%% src <id> <path>#<symbol>`),');
  console.log('register the container in root.mmd if it is a new top-level module, then re-run `npm run flowmap:ship`.');
  process.exit(1);
}
console.log(`flowmap-coverage: PASS — all ${sources.length} source files are represented in the map.`);
process.exit(0);
