...
/* =====================================================================
   ...
   novakai planning state is incoherent. Two claims, both machine-checked
   by plan-check.mjs (REAL-IDS, DANGLING-DEP, ACYCLIC, PARENT-EXISTS,
   COHERENT-ACCEPTED):

     • plan text carries NOVAKAI-PLAN:<path> -> that plan file must
       resolve and pass plan-check              -> ALLOW / DENY
     • NEAR-MISS sentinel (NOVAKAI_PLAN, wrong case) -> DENY — a typo'd
       sentinel is an attempted novakai plan, not prose (the AUD2 A1
...
const PLAN_CHECK = join(HERE, '..', 'plan', 'plan-check.mjs');
const SENTINEL = /NOVAKAI-PLAN:\s*(\S+)/;
// Compact token forms only — space-separated prose ("the novakai plan") is not a near-miss.
const NEAR_MISS = /NOVAKAI[-_]PLAN/i;
...
function deny(reason) {
  ...
  process.stderr.write('novakai plan-gate DENIED plan approval: ' + reason + '\n');
  process.exit(2);
}
...
if (NEAR_MISS.test(planText)) {
  deny('near-miss plan sentinel in the plan text (typo?) — use exactly NOVAKAI-PLAN:<path/to/plan.json>');
}
