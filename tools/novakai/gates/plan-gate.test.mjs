...
test('DENY: NOVAKAI-PLAN sentinel with an unresolvable path (exit 2)', () => {
  const dir = mkroot({ plan: 'none' });
  try {
    const r = gate({ tool_name: 'ExitPlanMode', tool_input: { plan: 'Build per NOVAKAI-PLAN:docs/novakai/plans/no-such.plan.json' } },
      { NOVAKAI_ROOT: dir });
    assert.equal(r.status, 2);
  } finally { rmSync(dir, { recursive: true, force: true }); }
});
...
test('ALLOW: NOVAKAI-PLAN sentinel pointing at a coherent plan (exit 0)', () => {
  ...
  try {
    ...
    const r = gate({ tool_name: 'ExitPlanMode', tool_input: { plan: 'Build per NOVAKAI-PLAN:the.plan.json' } },
      { NOVAKAI_ROOT: dir });
    assert.equal(r.status, 0, r.stdout + r.stderr);
  } finally { rmSync(dir, { recursive: true, force: true }); }
});
...
test('DENY: near-miss sentinel NOVAKAI_PLAN (underscore typo) blocks (exit 2)', () => {
  const dir = mkroot({ plan: 'none' });
  try {
    const r = gate({ tool_name: 'ExitPlanMode', tool_input: { plan: 'Build per NOVAKAI_PLAN: the.plan.json' } },
      { NOVAKAI_ROOT: dir });
    assert.equal(r.status, 2);
...
test('ALLOW: prose "novakai plan" (space-separated words) is not a near-miss (exit 0)', () => {
  const dir = mkroot({ plan: 'none' });
  try {
    const r = gate({ tool_name: 'ExitPlanMode', tool_input: { plan: 'discuss how the novakai plan loop works' } },
      { NOVAKAI_ROOT: dir });
    assert.equal(r.status, 0);
  } finally { rmSync(dir, { recursive: true, force: true }); }
});
