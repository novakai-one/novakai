...
function ship(cwd) {
  const r = spawnSync('node', [STAMP_CLI], { cwd, encoding: 'utf8', env: { ...process.env, NOVAKAI_ROOT: cwd } });
...
test('DENY: src committed after the stamp blocks the stop (exit 2, names novakai:ship)', () => {
  ...
  try {
    ...
    assert.match(r.stderr, /novakai:ship/);
  } finally { rmSync(dir, { recursive: true, force: true }); }
});
...
test('M2b: the block/fresh decisions are metered; the anti-loop passthrough is NOT (exit codes unchanged)', () => {
  const readLog = (dir) => {
    const p = join(dir, 'docs', 'novakai', 'metrics', 'session-log.jsonl');
