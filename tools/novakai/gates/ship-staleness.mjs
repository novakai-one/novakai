...
/* =====================================================================
   ...
   hook"): the hook detects staleness and DEMANDS `npm run novakai:ship`
   ...
   2026-07-04, KNOWN_EDGES.md): `novakai:ship` writes docs/novakai/
   ...
   so the stamp always has something new to write and commit.
     ...
     • stamp hash !== current src/ hash  -> stale (src changed, uncommitted
       or committed, since the last novakai:ship).
   ...
   the payload, decide). NOVAKAI_ROOT env var is the test seam.
...
try {
  ...
  if (stale) {
    record('deny', 'src/ content has changed since the last novakai:ship — re-sync (npm run novakai:ship)');
    process.stderr.write(
      'novakai ship-staleness BLOCKED the stop: src/ content has changed since the last novakai:ship\n' +
      '(docs/novakai/ship-stamp.json is out of date).\n' +
      'Re-sync before ending the session (protocol rule 5): npm run novakai:ship — then commit the\n' +
