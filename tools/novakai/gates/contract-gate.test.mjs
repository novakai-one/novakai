...
test('ALLOW: spawn carrying a VALID, coherent contract id (exit 0)', () => {
  const r = gate({ tool_name: 'Agent', tool_input: { prompt: 'Implement this. NOVAKAI-CONTRACT:frame-transform' } });
  assert.equal(r.status, 0);
});

test('DENY: sentinel with an unresolvable contract id (exit 2), from the PRIMARY deny branch', () => {
  const r = gate({ tool_name: 'Agent', tool_input: { prompt: 'Implement. NOVAKAI-CONTRACT:no-such-change-xyz' } });
...
test('DENY (F-01): near-miss sentinel NOVAKAI_CONTRACT (underscore typo) blocks (exit 2)', () => {
  const r = gate({ tool_name: 'Agent', tool_input: { prompt: 'Implement this. NOVAKAI_CONTRACT: frame-transform' } });
  assert.equal(r.status, 2);
...
test('DENY (F-01): correct-cased sentinel with no id blocks (exit 2)', () => {
  const r = gate({ tool_name: 'Agent', tool_input: { prompt: 'Implement per NOVAKAI-CONTRACT protocol' } });
  assert.equal(r.status, 2);
});

test('ALLOW: prose "novakai contract" (space-separated words) is not a near-miss (exit 0)', () => {
  const r = gate({ tool_name: 'Agent', tool_input: { prompt: 'read how the novakai contract loop works and report' } });
  assert.equal(r.status, 0);
});
...
test('M2b: every decision is metered — deny/allow write gate events, exit codes unchanged', () => {
  ...
  const d = gate({ tool_name: 'Agent', tool_input: { prompt: 'Implement. NOVAKAI-CONTRACT:no-such-change-xyz' } });
