...
/* =====================================================================
   ...
   The one-free-retry marker (<ROOT>/.novakai-turn-gate.json, or
   .novakai-turn-gate-<agentId>.json inside a subagent sidechain — see
...
try {
  if (existsSync(join(ROOT, '.novakai-gate-debug')))
    appendFileSync(join(ROOT, '.novakai-gate-debug.jsonl'), JSON.stringify(payload) + '\n');
...
const MARKER = join(ROOT, agentId ? `.novakai-turn-gate-${agentId}.json` : '.novakai-turn-gate.json');
