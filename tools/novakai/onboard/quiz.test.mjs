...
function quiz(cwd, args) {
  ...
  const r = spawnSync('node', [CLI, ...args], { cwd, encoding: 'utf8', env: { ...process.env, NOVAKAI_ROOT: cwd } });
...
test('check DENIES wrong answers (exit 1) and writes NO pass artifact', () => {
  ...
  try {
    ...
    assert.equal(existsSync(join(dir, '.novakai-quiz-pass.json')), false,
...
test('check on 100% exits 0 and emits a pass artifact bound to the map hash (F-03)', () => {
  ...
  try {
    ...
    const r = quiz(dir, ['check', '--answers', 'answers.json', ...MAP_ARGS]);
    assert.equal(r.status, 0, r.stdout);
    const pass = JSON.parse(readFileSync(join(dir, '.novakai-quiz-pass.json'), 'utf8'));
...
test('verify: a tampered artifact claiming a partial score is DENIED', () => {
  const dir = mkfixture();
  try {
    writeFileSync(join(dir, '.novakai-quiz-pass.json'),
...
test('session: check records --session (flag beats env; neither -> null)', () => {
  ...
  try {
    ...
    assert.equal(JSON.parse(readFileSync(join(dir, '.novakai-quiz-pass.json'), 'utf8')).session, 'flag-sess',
      'the explicit flag must beat the env');

    assert.equal(run([], { CLAUDE_CODE_SESSION_ID: 'env-sess' }).status, 0);
    assert.equal(JSON.parse(readFileSync(join(dir, '.novakai-quiz-pass.json'), 'utf8')).session, 'env-sess',
      'the harness env is the default identity');

    assert.equal(run([], { CLAUDE_CODE_SESSION_ID: '' }).status, 0);
    assert.equal(JSON.parse(readFileSync(join(dir, '.novakai-quiz-pass.json'), 'utf8')).session, null,
...
test('session: verify --session matches / mismatches / legacy artifact; flagless stays hash-only', () => {
  ...
  try {
    ...
    const pass = JSON.parse(readFileSync(join(dir, '.novakai-quiz-pass.json'), 'utf8'));
    delete pass.session;
    writeFileSync(join(dir, '.novakai-quiz-pass.json'), JSON.stringify(pass));
...
const readPass = (dir) => JSON.parse(readFileSync(join(dir, '.novakai-quiz-pass.json'), 'utf8'));
const writePass = (dir, p) => writeFileSync(join(dir, '.novakai-quiz-pass.json'), JSON.stringify(p));
