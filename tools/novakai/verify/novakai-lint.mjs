...
export function lint(text) {
  ...
  if (real.length >= FLAT_FAIL_MIN_NODES && drill.length === 0)
    fails.push(`FLAT: ${real.length} nodes, 0 drilled units. A repo novakai at architecture altitude ` +
...
if (import.meta.url === __invoked) {
  ...
  if (!path) { console.error('usage: novakai-lint <file.mmd> [--report]'); process.exit(2); }
