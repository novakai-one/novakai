#!/usr/bin/env node
/* =====================================================================
   trust-report.mjs — classify every claim in docs/novakai/_bundle.mmd
   into trust tiers so a reader knows what is machine-PROVEN vs prose.

   Usage:
     node tools/novakai/verify/trust-report.mjs          # human-readable
     node tools/novakai/verify/trust-report.mjs --json   # machine-readable

   Exit: 0 always (pure reporting, never a gate).

   Trust tiers:
     VERIFIED          — enforced by novakai:gate; mismatch blocks CI.
...
const BUNDLE = resolve(__dir, '../../../docs/novakai/_bundle.mmd');
