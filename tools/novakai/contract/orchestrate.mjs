...
/* =====================================================================
   ...
   For a plan it:
     ...
     2. for each dispatchable change, provisions an ISOLATED git worktree
        ...
        and drops that change's novakai:contract packet into it as
        CONTRACT.json — then tears the worktree down (--keep-worktrees to
        inspect);
     3. routes to novakai:verify-change (--strict aware) for each change's
        data-only verdict;
...
const WT_BASE = join(tmpdir(), 'novakai-orchestrate-wt');
