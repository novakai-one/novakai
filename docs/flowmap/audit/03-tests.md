...
Phase AUD3 (`docs/flowmap/audit/WORK_ORDER.md`). Per gate script: does its test exercise the
**DENY path** (the gate blocking bad input) or only ALLOW? Then a 3-predicate mutation spot-check
against `npm run spec:test:all`. **No fixes** — findings feed AUD4/AUD5. Status: `npm run flowmap:audit`.
...
- "in CI?" is read from `.github/workflows/spec-gate.yml` (jobs `buildspec-tests` + `flowmap-drift`);
...
| `flowmap-lint.mjs` | `flowmap-lint.test.mjs` + `.discriminate.test.mjs` | fn (`lint`), exit mapping reimplemented in the test | deny covered at logic level — FLAT (T2 L45–50), LOOSE-BAG (T4 L69–83), rejected file-mirror fixture (discriminate L21) | ✓ | **✗ tests not in CI** (CI runs the CLI on the real map) |
...
`grep -n "DENY: sentinel" tools/flowmap/contract-gate.test.mjs` ·
`grep -n "spawnSync\|execFileSync\|from './" tools/flowmap/handoff-fresh.test.mjs` (shows fn-only import, no CLI spawn) ·
...
mutation: `tools/flowmap/handoff-fresh.mjs:146` `if (codeTs > handoffTs)` → `if (false && codeTs > handoffTs)` (the F4 freshness gate can never fire).
repro:
```
sed -i '' 's/if (codeTs > handoffTs)/if (false \&\& codeTs > handoffTs)/' tools/flowmap/handoff-fresh.mjs
npm run spec:test:all            # observed: tests 166 · pass 166 · fail 0
npm run flowmap:handoff:check; echo $?   # observed: "✓ handoff is at least as fresh…" · 0 (vacuous gate)
git checkout -- tools/flowmap/handoff-fresh.mjs
```
...
mutation: `tools/flowmap/contract-gate.mjs:75` `if (r.status !== 0)` → `if (false && r.status !== 0)`.
repro:
```
printf '{"tool_name":"Agent","tool_input":{"prompt":"FLOWMAP-CONTRACT: no-such-change"}}' | node tools/flowmap/contract-gate.mjs; echo $?
```
...
repro: apply, then `node --test tools/flowmap/contract-gate.test.mjs`.
...
mutation: `tools/flowmap/roadmap.mjs:67` `pass: existsSync(resolve(c.path))` → `pass: true`.
repro:
```
printf '{"items":[{"id":"X1","phase":"X","title":"ghost doc","checks":[{"kind":"file","path":"docs/DOES-NOT-EXIST.md"}]}]}' > /tmp/ghost-roadmap.json
node tools/flowmap/roadmap.mjs --roadmap /tmp/ghost-roadmap.json; echo $?
```
...
`flowmap:roadmap:audit` CI step, **and this audit's own phase status** (`flowmap:audit`).
...
| T4 | fn-only tests (plan-check, plan-cert, gate.mjs drift logic, flowmap-lint, handoff-fresh) never spawn their CLIs — argv/exit wiring of five gates is mutation-blind | — |
