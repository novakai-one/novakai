...
Phase AUD2 (`docs/flowmap/audit/WORK_ORDER.md`). Attempt to break every GATE from `01-claims.md`.
Each entry: attack · `repro:` · observed result · verdict (HELD / BROKEN / PARTIAL / UNDETERMINED).
**No fixes** — findings feed AUD4/AUD5. Status: `npm run flowmap:audit`.
...
`repro:`
```
printf '{"tool_name":"Agent","tool_input":{"prompt":"FLOWMAP_CONTRACT: frame-transform"}}' | node tools/flowmap/contract-gate.mjs; echo $?   # typo → 0
printf '{"tool_name":"Agent","tool_input":{"prompt":"FLOWMAP-CONTRACT: no-such-change"}}'  | node tools/flowmap/contract-gate.mjs; echo $?   # unresolvable → 2 DENY
echo 'not json' | node tools/flowmap/contract-gate.mjs; echo $?                                                                             # malformed → 0
printf '{"tool_name":"Agent","tool_input":{}}' | node tools/flowmap/contract-gate.mjs; echo $?                                              # empty → 0
printf '{"tool_name":"Agent","tool_input":{"prompt":"go build"}}' | node tools/flowmap/contract-gate.mjs; echo $?                           # absent → 0
```
observed:
- Sentinel typo `FLOWMAP_CONTRACT` (underscore) → **exit 0** (ungated spawn).
- Correct `FLOWMAP-CONTRACT:` + unresolvable id → **exit 2**, `deny` JSON + stderr (deny path works).
...
`repro:` `grep -n contract-gate.test .github/workflows/spec-gate.yml` → **`61: - run: node --test tools/flowmap/contract-gate.test.mjs`**.
...
node tools/flowmap/handoff-fresh.mjs --check; echo $?                          # baseline clean → 0
printf '\n//x\n' >> src/main.ts && git commit -qam code; node …handoff-fresh --check; echo $?   # code newer → 1
printf '\n<!--x-->\n' >> docs/flowmap/SESSION_HANDOFF.md; node …handoff-fresh --check; echo $?    # handoff DIRTY → 0
...
node tools/flowmap/quiz.mjs check --answers .quiz-answers.json --seed 1; echo $?
git ls-files --error-unmatch .quiz-answers.json                      # → tracked
```
observed:
- Quiz appears in **no** workflow and **no** hook. `flowmap:onboard` only *prints* the quiz
...
node tools/flowmap/roadmap.mjs --roadmap hollow-roadmap.json   # H0(file)+H1(grep) → both BUILT
node tools/flowmap/roadmap.mjs --roadmap hollow-roadmap.json >/dev/null; echo $?   # → 0
```
...
node tools/flowmap/roadmap.mjs --audit-doc evade.md; echo $?     # table "done ✅", "Status — shipped", <div>state: built</div> → 0
# false-positive: a doc that QUOTES the banned pattern
node tools/flowmap/roadmap.mjs --audit-doc docs/flowmap/audit/01-claims.md; echo $?   # → 1
```
observed:
...
`repro:`
```
# CI-only steps absent from the local flowmap:verify chain
for s in handoff:check cert plan-check roadmap:audit acceptance; do \
  grep -c "flowmap:$s" .github/workflows/spec-gate.yml; \
  node -e "console.log(require('./package.json').scripts['flowmap:verify'].includes('$s'))"; done
# local-only tests absent from CI
for t in slice-core.test flowmap-lint.test flowmap-lint.discriminate.test tooling-map.test; do \
...
- **CI-only, not locally forced:** `handoff:check`, `cert`, `plan-check`, `roadmap:audit`,
  `acceptance` run in CI but are **not** in the local `flowmap:verify`/`ship` chain — a developer
  who runs `flowmap:ship` never exercises them.
- **Local-only, not in CI:** `slice-core.test`, `flowmap-lint.test`, `flowmap-lint.discriminate.test`,
  and the whole tooling-map chain (`tooling-map.test`, `tooling-coverage`, `flowmap:tooling:verify`)
  → CI count **0**. The tooling map (Phase I) is verified in no CI job.
- **Path-trigger gap:** `spec-gate.yml` fires only on `src/**`, `docs/flowmap/**`, `tools/**`, the
...
- **Crash-skip (BROKEN):** the Stop hook (handoff-fresh nudge) fires only on a *clean* Stop. Two
  ...
  reminder entirely.
- onboard-as-gate (CLM-047): `flowmap:onboard` exits 1 on a stale map, but whether SessionStart
