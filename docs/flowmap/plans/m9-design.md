# M9 — End-to-end flowmap testing: design plan
...
| 19 | Session close: `npm run flowmap:ship`; update `SESSION_HANDOFF.md`; `npm run flowmap:handoff:check` | ship chain + handoff-fresh.mjs (F4/H5) | Re-sync of the *real* repo + the meta-loop freshness gate. Deliberately not re-run inside the m9 test — already a CI predicate in spec-gate.yml; duplicating would double-fail every stale-handoff PR. | ship `DONE:` line; handoff:check exit 0 | handoff:check exit 1 → session ended with handover lagging code | CI spec-gate run + `docs/flowmap/ship-stamp.json` | Agent-protocol (CI-enforced) |
| 20 | `npm run flowmap:mvp` — M9 predicates computed | roadmap.mjs vs updated mvp-roadmap.json M9 entry | Status must be computed, never written: M9 "built" is itself a predicate set (exit criteria below). | M9 auto checks pass (`partial` until the demo recording exists, per statusRule — the honest verdict) | missing → predicates don't match shipped artifacts | roadmap output | Automated |
...
|---|---|---|---|
| tools/flowmap/onboard.mjs | step 1 | used | the loop's door (B1) |
| tools/flowmap/quiz.mjs | steps 2, 3 | used | Keystone 1; automated half = the refusal path |
| tools/flowmap/plan-check.mjs | steps 4, 18 | used | first gate; also the red-chain blocker |
| tools/flowmap/plan-cert.mjs | step 5 (+ `applyPlanToSpec` imported by approve-export) | used | pre-review certificate |
| tools/flowmap/approve-export.mjs | steps 6 (red), 8 (green) | used | first E2E exercise of the `--accepted-only` verdict path |
| tools/flowmap/status.mjs | step 9; routed by verify-change (11, 15) | used | structural half of every verdict |
| tools/flowmap/waves.mjs | step 10; routed by orchestrate (16) | used | dispatch schedule |
| tools/flowmap/contract.mjs | step 10; routed by orchestrate (16) | used | subagent packet |
| tools/flowmap/verify-change.mjs | steps 11, 15, 17 | used | the FAIL→PASS flip — M9's centerpiece |
...
| tools/flowmap/bundle.mjs | step 14 | used | mid-loop re-sync in sandbox |
| tools/flowmap/validate.mjs | steps 0, 14 | used | grammar gate on regenerated bundle |
...
| tools/flowmap/orchestrate.mjs | step 16; step 0 borrows its worktree mechanism | used | M9 closes its documented "verdict outside the worktree" gap |
| tools/flowmap/replay.mjs | step 17 | used | determinism of the verdict-bearing step |
| tools/flowmap/edge-verify.mjs | step 18 chain tail (existing F5 assertion) | used | re-sync guard |
| tools/flowmap/handoff-fresh.mjs | step 19 | used (CI/agent-protocol) | F4 gate; deliberately not re-run in-test |
| tools/flowmap/ship-stamp.mjs, ship-staleness.mjs | step 19 | used (agent-protocol) | re-sync stamp + Stop hook |
| tools/flowmap/{flowmap-lint,coverage,exports-coverage}.mjs | step 19 ship chain (+ step 1 via onboard) | used | map-quality gates, already CI-chained |
| tools/flowmap/roadmap.mjs | step 20 | used | computes M9's own built-ness |
| tools/flowmap/{edit-gate,plan-gate,turn-gate,contract-gate,reminder-hook}.mjs, turns.mjs | fire live in demo sessions (3, 7, 19) | used (agent-protocol) | .claude/settings.json hooks; logic unit-tested |
| tools/flowmap/lib/{canonical,metrics-log,src-tree-hash,transcript}.mjs | hashing/metrics seams under steps 10–17 (FLOWMAP_ROOT sink pattern reused from F5) | used | infra |
| tools/flowmap/metrics.mjs | wraps ship (19); sink-redirected in-test | used | M2b seam |
| tools/flowmap/trust-report.mjs | not exercised | unrelated | A4 surface, orthogonal to the loop spine |
| tools/flowmap/mutate.mjs + mutations.json | not exercised | unrelated | gate-mutation harness, own roadmap item |
| tools/flowmap/tooling-coverage.mjs | not exercised | unrelated | I1; extending loop-e2e adds no new module → no tooling-map churn |
| tools/flowmap/frag-check.mjs | not exercised | **unused in M9 — audited manual instrument** | no script/importer/hook, but it is a recorded curation-allowlist exclusion (tooling-curation-allowlist.txt: "build-time CONTRACT instrument for authoring THIS map", CLM-044). Not unaccounted; do not silently delete — either wire it or leave the audited verdict standing |
...
| tools/flowmap/reminder-hook.test.mjs | not in M9 steps — **and not in spec:test:all** | used (after fix) | live hook whose test never runs in the suite; one-line package.json fix (build step 4) |
...
| tools/flowmap/fixtures/*.mmd (4), tools/buildspec/__fixtures__/ (grouped) | lint/pipeline unit-test fixtures | used | unrelated to M9 |
| tools/flowmap/*.flowmap.mmd + tools/buildspec/buildspec.flowmap.mmd (6 fragments) | tooling self-map (I1) | used | untouched by M9 (no new module) |
| tools/{BUILD_FLOWMAP,DISTRIBUTION,README,SYNTAX_README}.md, tools/package.json | docs/packaging | unrelated | no M9 role |

## Build checklist (ordered — one new fixture, one extended file)

1. **[NEW] `docs/flowmap/plans/m9-loop.plan.json`** — one `add` change `m9-probe` (pure fn in `src/core/state/state.ts`), with `fm` + 3 acceptance cases + `verdicts:{"m9-probe":"accept"}` + **`newNode:{kind:"function",label,parent}`** (mandatory — see Core decisions). Sanity: `plan-check` and `plan-cert` green against HEAD.
2. **Extend `tools/flowmap/loop-e2e.test.mjs`** — sandbox helper (worktree at HEAD + node_modules symlink; reuse the existing METRICS_SINK pattern) + the M9 chain (steps 0, 2, 4–17) as new test blocks. No new npm script; `flowmap:loop` and `spec:test:all` already run it. Sandbox feasibility is verified: the verdict-bearing tools derive ROOT from `import.meta.url` (running the worktree's copies pins ROOT to the worktree); `quiz.mjs` and direct `extract` symbol resolution are cwd-relative, so **cwd = worktree is load-bearing, not optional**.
...
4. Add `tools/flowmap/reminder-hook.test.mjs` to `spec:test:all` (fixes the live-hook-untested anomaly).
5. Update `docs/flowmap/mvp-roadmap.json` M9 `checks` to the predicate set below.
6. `npm run flowmap:loop` + `npm run spec:test:all` green → ship + handoff (step 19).
...
```json
"checks": [
  { "kind": "file", "path": "docs/flowmap/plans/m9-loop.plan.json" },
  { "kind": "grep", "path": "docs/flowmap/plans/m9-loop.plan.json", "pattern": "\"verdicts\"" },
  { "kind": "grep", "path": "tools/flowmap/loop-e2e.test.mjs", "pattern": "m9-loop" },
  { "kind": "grep", "path": "tools/flowmap/loop-e2e.test.mjs", "pattern": "FAIL.*PASS|red.*green" },
  { "kind": "cmd",  "run": "npm run --silent flowmap:loop" },
  { "kind": "manual", "note": "Recorded demo of the agent-protocol steps (session-bound quiz pass + browser verdict review); automated spine is flowmap:loop." }
]
```
...
2. **CI runtime** — the M9 chain adds ~2 tsc+ts-morph passes to loop-e2e (likely +1–2 min), and the proposed `cmd` predicate makes every `flowmap:mvp` run re-execute the full loop-e2e suite. Accept, or split to `m9-e2e.test.mjs` later (reversible; grep predicates move with it).
...
6. **Step 1 caveat** — `flowmap:onboard` attests the main *working tree*; the sandbox is at *HEAD*. Identical only on a clean checkout (always true in CI); locally a dirty tree makes "one run attests both" false. The test should assert a clean tree or scope the claim to CI.

## Verification

- Automated spine: `npm run flowmap:loop` green (all M9 test blocks incl. red paths), then `npm run spec:test:all` (no regressions), then `npm run flowmap:mvp` shows M9 auto-predicates green (`partial` overall until the demo recording exists — the honest verdict per statusRule).
- Determinism: step 17 replay inside the test run itself.
- 0-context confirmation (per session protocol §3): a fresh agent runs `npm run flowmap:loop` + `npm run flowmap:mvp` and confirms M9 from output alone — never from the builder's account.
...
|---|---|---|---|
| 1 | `quiz.mjs verify` fail-closed with no pass artifact | CONFIRMED | `quiz.mjs:305-308` — `if (!existsSync(PASS_FILE)) { … exit(1) }`; `.flowmap-quiz-pass.json` gitignored (`.gitignore:26`), fresh worktree has none |
...
| 9 | Probe host `src/core/state.ts` | **REFUTED on path → fixed** | Module is `src/core/state/state.ts` + colocated `src/core/state/state.flowmap.mmd` (21 `%% src` lines in the needed format). Right host, wrong path — corrected |
| 10 | loop-e2e runs under `flowmap:loop` + `spec:test:all` + CI | CONFIRMED | package.json both scripts; `.github/workflows/spec-gate.yml:31` runs `spec:test:all` |
...
1. Materialize this document in the repo as `docs/flowmap/plans/m9-design.md` (committed, so a 0-context session can read the design without this plan file), on a feature branch — never on main.
2. Execute the build checklist above.
3. Close the session per protocol §5: `npm run flowmap:ship`, then update `docs/flowmap/SESSION_HANDOFF.md` command-anchored (every claim = a command: `npm run flowmap:loop`, `npm run flowmap:mvp`, `node --test tools/flowmap/loop-e2e.test.mjs`), then `npm run flowmap:handoff:check` green — the clean handover for the next session.

## Critical files

- `tools/flowmap/loop-e2e.test.mjs` — host of the M9 chain (the one file extended)
- `docs/flowmap/plans/m9-loop.plan.json` — [NEW] fixture plan driving the red→green flip
- `tools/flowmap/verify-change.mjs` — the verdict whose FAIL→PASS flip M9 exists to prove
- `tools/flowmap/orchestrate.mjs` — worktree mechanism reused; its documented gap is what M9 closes
- `docs/flowmap/mvp-roadmap.json` + `package.json` — M9 predicate set + reminder-hook suite fix
