...
- **Browser leg (human-driven: planner review/approve, unfold verifying the
  ...
  Save as `docs/flowmap/demo/browser-recording.mov`. **Genuine mouse input
...
- **CLI legs (onboard, quiz, plan-check, approve, contract, verify-change,
  ...
  ```
  script -q docs/flowmap/demo/cli-session.typescript
  ...
  per-command `tee`: `npm run flowmap:onboard | tee docs/flowmap/demo/logs/onboard.log`
...
## (b) Artifact set — `docs/flowmap/demo/` after the run
...
| `quiz-pass.json` | `npm run flowmap:onboard` then `npm run flowmap:quiz -- --check` (100%) → `cp .flowmap-quiz-pass.json docs/flowmap/demo/quiz-pass.json` |
| `english-request.txt` | the exact sentence handed to the demo agent (verbatim, no paraphrase) |
| `plan.json` | the demo agent's freshly hand-authored plan — **its own file** (e.g. `docs/flowmap/plans/m9-demo.plan.json`), never `public/plan.json` (pitfall below) |
| `plan-check.json` | `npm run flowmap:plan-check -- --plan docs/flowmap/demo/plan.json --json > docs/flowmap/demo/plan-check.json` |
...
| `approve-export/` | `npm run flowmap:approve -- --plan docs/flowmap/demo/approved-plan.json --out docs/flowmap/demo/approve-export --accepted-only` → contains `approved.mmd`, `contracts/`, `plan.json`, `CHECKLIST.md` |
| `approved-spec.mmd` | `cp docs/flowmap/demo/approve-export/approved.mmd docs/flowmap/demo/approved-spec.mmd` (stable top-level name for the exported spec; byte-identical to the file inside `approve-export/`) |
| `contracts/<id>.contract.json` | one per accepted change: `npm run flowmap:contract -- --change <id> --plan docs/flowmap/demo/approved-plan.json --json > docs/flowmap/demo/contracts/<id>.contract.json` |
| `verify-change/<id>.pre.json` | run BEFORE implementing, expected `verdict: "FAIL"` (red): `npm run flowmap:verify-change -- --change <id> --plan docs/flowmap/demo/approved-plan.json --strict --json > docs/flowmap/demo/verify-change/<id>.pre.json` |
...
| `orchestrate.json` | `npm run flowmap:orchestrate -- --plan docs/flowmap/demo/approved-plan.json --strict --json > docs/flowmap/demo/orchestrate.json` |
| `acceptance.json` | `npm run flowmap:acceptance -- --plan docs/flowmap/demo/approved-plan.json --json > docs/flowmap/demo/acceptance.json` (post-implementation, all cases green) |
| `writeback.diff` | `git diff -- '*.flowmap.mmd' > docs/flowmap/demo/writeback.diff` captured immediately after `npm run flowmap:writeback` |
| `ship.log` | `npm run flowmap:ship 2>&1 \| tee docs/flowmap/demo/ship.log` — must end with the `DONE: ...` line `flowmap:ship:steps` prints on success |
...
`docs/flowmap/mvp-roadmap.json`'s M9 item today has a single `manual` check
...
```json
{
  "id": "M9",
  "checks": [
    { "kind": "file", "path": "docs/flowmap/demo/manifest.json" },
    { "kind": "file", "path": "docs/flowmap/demo/quiz-pass.json" },
    { "kind": "grep", "path": "docs/flowmap/demo/quiz-pass.json", "pattern": "\"score\": \"12/12\"" },
    { "kind": "file", "path": "docs/flowmap/demo/english-request.txt" },
    { "kind": "file", "path": "docs/flowmap/demo/plan.json" },
    { "kind": "cmd", "run": "npm run -s flowmap:plan-check -- --plan docs/flowmap/demo/plan.json" },
    { "kind": "file", "path": "docs/flowmap/demo/approved-plan.json" },
    { "kind": "grep", "path": "docs/flowmap/demo/approved-plan.json", "pattern": "\"verdicts\"" },
    { "kind": "file", "path": "docs/flowmap/demo/approve-export/approved.mmd" },
    { "kind": "file", "path": "docs/flowmap/demo/approve-export/CHECKLIST.md" },
    { "kind": "file", "path": "docs/flowmap/demo/orchestrate.json" },
    { "kind": "grep", "path": "docs/flowmap/demo/orchestrate.json", "pattern": "\"fail\":0" },
    { "kind": "file", "path": "docs/flowmap/demo/acceptance.json" },
    { "kind": "cmd", "run": "! grep -q '\"pass\":false' docs/flowmap/demo/acceptance.json" },
    { "kind": "file", "path": "docs/flowmap/demo/writeback.diff" },
    { "kind": "file", "path": "docs/flowmap/demo/ship.log" },
    { "kind": "grep", "path": "docs/flowmap/demo/ship.log", "pattern": "^DONE:" },
    { "kind": "file", "path": "docs/flowmap/demo/browser-recording.mov" }
  ]
}
```

Notes on the two `grep`/`cmd` verdict checks: `orchestrate.json`'s summary
field is `{ total, pass, passUnproven, fail }` (see `tools/flowmap/orchestrate.mjs`
...
1. **Genuine mouse input only.** `unfold.ts` (~line 952) calls
   ...
   2026-07-03, `docs/flowmap/probes/m5-tabs2-verbs.probe.js` header). A human
...
3. **The demo runs on its own fresh plan, never `public/plan.json`.**
   `public/plan.json` is the plan CI's `flowmap:verify:full` targets
   (`npm run flowmap:cert -- --plan public/plan.json`, etc.) — overwriting it
   for a one-off demo would break CI and corrupt a checked-in artifact. The
   demo agent authors and commits its plan under `docs/flowmap/plans/` (or
...
4. **Plan-lifecycle**: per KNOWN_EDGES, once an `add` change lands its row
   must be hand-flipped to `modify` to keep `flowmap:plan-check` green
...
5. **`.flowmap-quiz-pass.json` is gitignored, session-scoped, and
   ...
   `flowmap:ship` re-sync at the end) after the quiz pass and before the copy,
   re-run `flowmap:quiz -- --check` to confirm it still matches, or copy it
