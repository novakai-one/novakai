...
Phase AUD4 (`docs/novakai/audit/WORK_ORDER.md`). Consolidates every BROKEN / PARTIAL /
...
one finding per plan.** No fixes here. Status: `npm run novakai:audit`.
...
|---|---|---|---|---|---|
| F-01 | keystone-bypass | CLM-037 "makes 'subagents go through the contract' a **100% GATE**, not a convention" — the gate is narrow: sentinel typo (`NOVAKAI_CONTRACT`), absent sentinel, malformed stdin, non-Agent tools all ALLOW; only exact-cased sentinel + unresolvable id DENIES. CLM-038 (fail-open) is accurate; the "100%" is not | `printf '{"tool_name":"Agent","tool_input":{"prompt":"NOVAKAI_CONTRACT: x"}}' \| node tools/novakai/contract-gate.mjs; echo $?` → 0 (A1) | Detect near-miss sentinels (case/underscore variants) and DENY or warn loudly; deny (not allow) an Agent/Task payload that fails to parse; fix the header to state the true scope. **Must ship with test changes: `contract-gate.test.mjs` currently asserts fail-open as required behavior (T3), and the primary deny branch is masked by the downstream unparseable-output deny (M2a) — test both paths explicitly** | M |
...
| F-04 | keystone-bypass | CLM-006/053 "Roadmap status is computed … recomputed every run, so it **cannot lie**" — a 0-byte file satisfies a `file` predicate, a lone grep token satisfies a `grep` predicate, status never exits non-zero (A5); and `roadmap.mjs` — the status computer, the `roadmap:audit` CI gate, and this audit's own predicate runner — has **no test**: hard-wiring `file`→true leaves the suite 166/166 (M3 SURVIVED, T2) | `03-tests.md` §2 M3 (ghost roadmap reads BUILT) · `grep -rn "roadmap.mjs" tools/novakai/*.test.mjs tools/buildspec/*.test.mjs` → empty | Author `roadmap.test.mjs` with deny fixtures (missing file, hollow file, failing cmd, audit-doc corpus); strengthen predicates where hollowness matters (min-content grep counts, `cmd` checks) — the audit phases' own predicates first | S–M |
...
| F-06 | gap | CLM-012/070 "CI enforces the whole loop" / ship's DONE banner — the CI and local gate chains diverge both directions: `handoff:check`, `cert`, `plan-check`, `roadmap:audit`, `acceptance` run in CI only (a dev running `novakai:ship` never exercises them); `slice-core`, both `novakai-lint` tests run locally only | `02-attacks.md` A7 (per-script grep loop) | Define one canonical gate list consumed by both `novakai:verify` and `spec-gate.yml` (or minimally: append the five CI-only steps to a `novakai:verify:full` and add the local-only tests to CI) | S |
...
| F-10 | gap | CLM-033/043/050 + F4 — five gate CLIs are mutation-blind: their tests import internals and never spawn the binary (`gate.mjs` — spawned by zero tests, `plan-check`, `plan-cert`, `novakai-lint`, `handoff-fresh`), so each script's argv/exit wiring is unverified (T4) | `grep -L spawnSync tools/novakai/plan-check.test.mjs tools/novakai/plan-cert.test.mjs tools/novakai/novakai-lint.test.mjs tools/novakai/handoff-fresh.test.mjs` → all four listed | One thin spawn-test per gate: bad input → assert documented non-zero exit; good input → 0 | S |
...
1. **F-19 first** (Chris, one setting: protect `main`, require `buildspec-tests` + `novakai-drift`) —
...
Per the work order: convert AUD5's `manual` check in `docs/novakai/audit/audit-roadmap.json` to
