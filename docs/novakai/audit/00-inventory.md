...
Phase AUD0 of the tooling audit (`docs/novakai/audit/WORK_ORDER.md`). This document ENUMERATES every
guarantee the tooling claims. It does not judge them (that is AUD1) or attack them (AUD2). Status is
computed: `npm run novakai:audit`.
...
| `novakai-lint.mjs` | semantic quality gate; rejects flat file-mirror maps; exit 1 on FAIL, warnings never fail | `novakai-lint.test.mjs`, `.discriminate.test.mjs` |
...
|---|---|---|
| PreToolUse | `Agent\|Task` | `node tools/novakai/contract-gate.mjs` |
| SessionStart | `startup\|resume` | `npm run --silent novakai:onboard` |
| Stop | *(none)* | `node tools/novakai/handoff-fresh.mjs` (bare nudge — NOT `--check`) |
...
|---|---|---|
| `spec-gate.yml` | `buildspec-tests` | `spec:test`, `spec:conformance`, then hard-coded `node --test` of: normtype, plan-layout, plan-from-diff, acceptance, writeback, plan-cert, edge-verify, plan-check, approve-export, loop-e2e, canonical, contract, verify-change, replay, **contract-gate**, waves, handoff-fresh, orchestrate; plus `novakai:acceptance -- --plan public/plan.json` |
| `spec-gate.yml` | `novakai-drift` | `novakai:bundle`, `git diff --exit-code _bundle.mmd`, validate, lint, coverage, exports, gate, edges, `roadmap:audit`, `cert --plan public/plan.json`, `plan-check --plan public/plan.json`, `handoff:check` |
| `deploy.yml` | `build` / `deploy` | `npm ci`, `npm run build` (`tsc --noEmit && vite build`), Pages deploy — **no novakai gates** |

Trigger for `spec-gate.yml`: push/PR on paths `src/**`, `docs/novakai/**`, `tools/**`, the workflow
file, `package.json`.
...
| script | chain |
|---|---|
| `novakai:verify` | bundle → validate → lint → coverage → exports → gate → edges (7 gates) |
| `novakai:ship` | `novakai:verify` chain + bodies + `echo 'DONE: …'` |
| `novakai:drift` | identical to `novakai:verify` |
| `novakai:tooling:verify` | tooling:bundle → validate → lint → tooling-coverage → tooling-map.test → `echo 'DONE: …'` |
...
| CLM-007 | "novakai:roadmap:audit — fails if any prose status marker creeps back into CLAUDE.md" | `roadmap.mjs --audit-doc` exit 1 | CLAUDE.md |
| CLM-008 | A2 "CI regenerates the node set + signatures from code and fails on divergence" | `novakai:gate` in CI | CLAUDE.md |
...
| CLM-013 | F2 "the harness runs novakai:onboard at session start, so onboarding is forced, not remembered" | SessionStart hook | CLAUDE.md |
...
| CLM-016 | I1 "proven by novakai:tooling:verify" | tooling:verify chain | CLAUDE.md |
| CLM-017 | "every claim is a command the next agent runs, not prose to trust" | command-anchored handoff | CLAUDE.md |
| CLM-018 | "novakai-lint fails the build if the map degrades into a flat file-mirror" | `novakai-lint` exit 1 | CLAUDE.md |
...
| CLM-036 | "Output FAILS novakai-lint by design" (`--init`) | lint exit 1 | buildspec/scaffold.mjs |
| CLM-037 | "makes 'subagents go through the contract' a 100% GATE, not a convention … DENY (exit 2)" | PreToolUse deny exit 2 | novakai/contract-gate.mjs |
| CLM-038 | "It FAILS OPEN" | any fault → ALLOW | novakai/contract-gate.mjs |
| CLM-039 | "byte-deterministic EXECUTION PACKET … ROUTES to existing computation — does not recompute" | canonical routing | novakai/contract.mjs |
| CLM-040 | "One uncovered file = exit 1" | exit 1 | novakai/coverage.mjs |
| CLM-041 | "the one layer the review's confidence rests on was the one layer nothing enforced" → edges accounted for | exit 1 on unaccounted edge | novakai/edge-verify.mjs |
| CLM-042 | "it FAILS unless the symbol is node/alias/allowlisted … 'omitted' an auditable decision, never an accident" | exit 1 | novakai/exports-coverage.mjs |
| CLM-043 | "Exit 1 on any FAIL; warnings never fail … separate a real architecture map from a flat file-mirror" | exit 1 | novakai/novakai-lint.mjs |
| CLM-044 | "pass/fail by machine, never by prose" | exit 0 required | novakai/frag-check.mjs |
| CLM-045 | "Always exits 0 — non-blocking. It is a nudge, not a trap … verifiable half is the F4 roadmap predicate" | exit 0 nudge (NOT a gate) | novakai/handoff-fresh.mjs |
| CLM-046 | "'100 subagents → 100 identical results' … enforced by replay.mjs … no Date.now/new Date/Math.random … no absolute paths" | determinism rules via replay | novakai/lib/canonical.mjs |
| CLM-047 | "PROVES the map is true + complete as of HEAD … Exit 0 = trustworthy, 1 = NOT" | exit 1 | novakai/onboard.mjs |
| CLM-048 | "NOTHING chained them. This routes them into one run" (H4) | chains waves→contract→verify | novakai/orchestrate.mjs |
| CLM-049 | "proves the plan is enforceable … Green = internally consistent … a human should never have wasted a review" | apply→stubs→tsc→gate | novakai/plan-cert.mjs |
| CLM-050 | "Exit: 0=coherent, 1=problems, 2=bad args" | exit 1 | novakai/plan-check.mjs |
| CLM-051 | "answer key is NEVER written to disk … so it cannot lie … A 100% score proves two things" | recompute key at scoring | novakai/quiz.mjs |
| CLM-052 | "replay is what PROVES [determinism] … leak detector run FIRST. Exit 0=deterministic, 1=divergence" | byte-identical assertion | novakai/replay.mjs |
| CLM-053 | "don't WRITE state, COMPUTE it … recomputed every run, so it cannot lie" + "--audit-doc … so CLAUDE.md can never silently drift again" | predicate + audit exit 1 | novakai/roadmap.mjs |
| CLM-054 | "must be a derived fact, never a prose note … recomputed from source every run, so it cannot go stale" | recompute vs current code | novakai/status.mjs |
| CLM-055 | "the tooling's proof of TRUE + COMPLETE … One unmapped module = exit 1 … One dangling pointer = exit 1" | exit 1 | novakai/tooling-coverage.mjs |
| CLM-056 | "VERIFIED — enforced by novakai:gate; mismatch blocks CI" / "Exit: 0 always (never a gate)" | reporting only (NOT a gate) | novakai/trust-report.mjs |
| CLM-057 | "Exit code 1 if any ERROR" | exit 1 | novakai/validate.mjs |
| CLM-058 | "what escapes is not its prose but THIS verdict — computed, not narrated … 100 byte-identical verdicts. PASS iff structural=='built' AND (no cases OR all green)" | closed-form data verdict | novakai/verify-change.mjs |
| CLM-059 | "Exit: 0=success, 2=bad … topological sort with cycle detection" | exit 2 on bad | novakai/waves.mjs |

### 2c. Echo / CI-string claims

| id | claim | claimed mechanism | file |
|---|---|---|---|
| CLM-070 | "DONE: map is grammar-valid + structurally sound (lint) + file-complete (coverage) + symbol-complete (exports) + in sync (gate) + edges code-backed-or-audited … bodies regenerated" | success banner after 7-gate chain | package.json `novakai:ship` |
| CLM-071 | "DONE: tooling map bundled + grammar-valid + architectural (lint) + complete & symbol-true + deterministic" | success banner after chain | package.json `novakai:tooling:verify` |
| CLM-072 | "_bundle.mmd is stale. Run novakai:ship and commit." (`::error::` + exit 1) | CI fails on stale bundle | spec-gate.yml |
| CLM-073 | "✓ CERTIFIED — no new compile errors or drift (apply→stubs→tsc→gate). Safe to send to human review." | printed on cert pass | novakai/plan-cert.mjs |
| CLM-074 | "✓ DETERMINISTIC — all N runs identical … the 100→100 proof … ROUTES to a deterministic command" | printed on determinism pass | novakai/replay.mjs |
| CLM-075 | "novakai contract-gate DENIED spawn: <reason>" | stderr on DENY (exit 2) | novakai/contract-gate.mjs |
| CLM-076 | "NOT verified — the map answers these deterministically; re-read _bundle.mmd" | printed on quiz fail (exit 1) | novakai/quiz.mjs |
| CLM-077 | "cannot read plan: … exit(2)" | exit 2 on bad input (many tools) | plan-cert/waves/contract/plan-check/… |
| CLM-078 | verify-change `--strict`: "PASS_UNPROVEN (and any non-PASS) exits non-zero; JSON body byte-identical" | strict-mode exit | novakai/verify-change.mjs |
...
- **`deploy.yml` runs zero novakai gates** — any "CI enforces" claim (CLM-012) is scoped to
  `spec-gate.yml` only.
- **Quiz is not wired to any hook or CI** (relevant to CLM-011): `novakai:onboard` prints the quiz
...
- Several composite gates route intermediates through `/tmp` (`novakai:gate`, `novakai:bodies`).
