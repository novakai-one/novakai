# NovaKai (novakai) — MVP build plan

Personal record only. Live status: `npm run novakai:mvp` (computed, cannot go stale).
MVP exit = recorded end-to-end run on a foreign repo (M9).

## P0 — Rename
- [ ] M0.1 Resolve naming collision with existing NovaKai repo
- [ ] M0.2 Rename novakai repo/package → novakai; sweep path refs in plans/handoffs

## P1 — Tooling enforceable (protects everything after)
- [ ] M1.1 AUD0 inventory (`docs/novakai/audit/00-inventory.md`)
...
- [ ] M2.3 Hook: Stop includes novakai:ship staleness check
