...
Scope note (recorded assumption): this plan touches only `docs/novakai/roadmap.json`
...
- `node -e "const s=require('./package.json').scripts['spec:test:all']; for (const f of ['tools/buildspec/acceptance.test.mjs','tools/buildspec/plan-layout.test.mjs','tools/novakai/loop-e2e.test.mjs']) console.log(f, s.includes(f))"`
  — all three suites are in the canonical list, so all three already run in CI.
- `node --test tools/novakai/gate-parity.test.mjs` — the suite that fails the build if
...
`docs/novakai/roadmap.json` only. Each stale single-link grep becomes a fail-closed
...
- E4: keep the `novakai:cert` grep; add the canonical-suite grep on spec-gate.yml; pin
  the real-plan acceptance step (`novakai:acceptance -- --plan public/plan.json`);
...
- F5: keep the file + cmd checks; replace the spec-gate.yml literal grep with the
  canonical-suite grep plus a package.json grep for `tools/novakai/loop-e2e.test.mjs`.

## Acceptance (run after the edit)

- `npm run novakai:roadmap` — 32 built, 0 partial (E4 5/5, F5 4/4).
- `node --test tools/novakai/roadmap.test.mjs tools/novakai/gate-parity.test.mjs` — pass.
- `npm run novakai:roadmap:audit` — both scans clean.
- `npm run novakai:ship` — DONE line; `git status` shows no regenerated drift.
