...
Build one real UX fix in `src/` through the full novakai loop —
...
1. **The lead is an orchestrator, nothing else.** The lead never opens a file under `src/`.
   The lead's inputs are: `docs/novakai/_bundle.mmd` (after passing its own quiz), command exit
...
- SessionStart already ran `novakai:onboard`; confirm it printed MAP TRUSTWORTHY.
- Pass your own quiz (session-bound; nobody else's pass attests your read):
  `npm run novakai:quiz -- generate --n 12 --seed <pick>` → answer from `_bundle.mmd` only →
  `npm run novakai:quiz -- check --answers <file> --seed <same>` → required: 100%.
...
`docs/novakai/plans/m10.plan.json` to the plan schema — per change: `id`, `status`, `target`,
...
`docs/novakai/demo/prep/plan.json`.
- Lead closes the stage with:
  `npm run novakai:plan-check -- --plan docs/novakai/plans/m10.plan.json` → exit 0
  `npm run novakai:cert -- --plan docs/novakai/plans/m10.plan.json` → prints CERTIFIED, exit 0
...
- Chris loads the plan in the novakai app and reviews the **visual diff and blast radius** on the
...
- Lead closes the stage:
  `npm run novakai:approve -- --plan docs/novakai/plans/m10.plan.json --out docs/novakai/plans/m10-approved --accepted-only` → exit 0.
  The emitted `approved.mmd` + `contracts/` are now the binding shape. Nothing before this stage
  touches `src/`.

### Stage 3 — schedule (lead)
- `npm run novakai:waves -- --plan docs/novakai/plans/m10.plan.json --strict --json` → wave 0 ids.

### Stage 4 — build (one sonnet subagent per change, 0 context)
For each change id in the current wave:
- Lead fetches the packet: `npm run novakai:contract -- --change <id> --plan docs/novakai/plans/m10.plan.json --json`.
- Spawn a builder whose prompt contains the sentinel line `NOVAKAI-CONTRACT:<id>` and
  `NOVAKAI-PLAN:docs/novakai/plans/m10.plan.json` (the PreToolUse gate validates the packet at
...
- The builder's first act is its own scoped quiz pass (the edit gate blocks `src/` writes
  without a fresh pass): `novakai:quiz -- generate --scope <module> ...` → check → 100%.
...
- `npm run novakai:verify-change -- --change <id> --plan docs/novakai/plans/m10.plan.json --strict --json`
...
- Scope check: `git diff --name-only <branch-base>..HEAD` must be a subset of {files named by the
  contract packet's `source` + blast-radius refs} ∪ {`docs/novakai/plans/m10*`}. Any other file →
  revert the builder's commit, record the violation in the manifest, respawn a fresh builder with
  the violation quoted.
- `npm run novakai:acceptance -- --plan docs/novakai/plans/m10.plan.json` → exit 0.
- Repeat Stages 3–5 until `novakai:waves` returns no unbuilt changes.

### Stage 6 — re-sync (lead runs; a sonnet subagent fixes anything red)
- `npm run novakai:writeback -- docs/novakai/plans/m10.plan.json --fragment <target fragment> --dry`
  first; then without `--dry` if the plan added nodes.
- `npm run novakai:ship` → exit 0, then `git status --porcelain` shows only intended changes —
...
- `npm run novakai:status -- --plan docs/novakai/plans/m10.plan.json` → exit 0 (all built, none
  drifted).
...
`npm run novakai:handoff:check`, and returns a command → exit-code table. The auditor never sees
...
`docs/novakai/plans/m10-run.json`, written by the lead as stages close:
...
- Update `docs/novakai/SESSION_HANDOFF.md`: every claim a command, per protocol §5.
