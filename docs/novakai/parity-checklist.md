...
(`docs/novakai/_bundle.mmd` descs + module edges), `public/bodies.json`
...
`npm run novakai:onboard`, then grep the units named in each row.
...
(`docs/novakai/mvp-roadmap.json`), not by hand-editing this file.
...
`docs/novakai/root.mmd`, `tools/buildspec/diff-workspace.smoke.mjs` (an orphaned smoke test,
...
- RESOLVED (2026-07-04, ruling 2026-07-04/2): `initDiffWorkspace` was never called anywhere in
  ...
  cleanup.
- RESOLVED (2026-07-04): `unfold -.->|routeGraph| avoidRouter` is present in `docs/novakai/root.mmd`
  and verified `VERIFIED(import)` by `npm run novakai:edges` — this bullet was stale prose from
