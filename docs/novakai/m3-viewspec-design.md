...
machine-checkable target (`npm run novakai:mvp` computes how much of it has landed — this doc
never says).
...
- `docs/novakai/plans/m3-viewspec.plan.json` — the plan artifact, the first in this repo to
  carry H1 projection acceptance cases.
- `tools/buildspec/viewspec.test.mjs` — properties too fiddly for JSON cases.
- Map fragments (`src/core/viewspec/viewspec.novakai.mmd`, `root.mmd`, unfold fragment) and the
  M3 predicate conversion in `docs/novakai/mvp-roadmap.json` (§6).
...
The M3 `checks` in `docs/novakai/mvp-roadmap.json` are replaced by (the `manual` note is dropped
...
```json
"checks": [
  { "kind": "file", "path": "docs/novakai/m3-viewspec-design.md", "minBytes": 2000 },
  ...
  { "kind": "cmd",  "run": "npm run novakai:acceptance -- --plan docs/novakai/plans/m3-viewspec.plan.json" },
...
`npm run novakai:mvp` computes where M3 stands against these checks at any moment — that
command, not this doc, is the status.

## 7. Build order in this PR (test-first)

1. **This doc** (commit 1 — review order is doc → code).
2. **Map fragments**: `src/core/viewspec/viewspec.novakai.mmd` (new module fragment: 3 type
   nodes + 3 function nodes with `%% src`/`%% fm:meta`), `docs/novakai/root.mmd` (module node,
...
3. **Plan** `docs/novakai/plans/m3-viewspec.plan.json`: `plan-check` green; `novakai:acceptance`
...
8. **Ship + prove**: `novakai:ship`, `spec:test:all`, `typecheck`, `novakai:mvp`,
   `novakai:roadmap:audit`.
