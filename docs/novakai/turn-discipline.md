...
> status. Every number quoted here is a snapshot of `docs/novakai/turn-baseline.json`
...
```
npm run --silent novakai:turns -- summary     # per-session table + medians, live
cat docs/novakai/turn-baseline.json           # frozen baseline + targets + protocol
```

## Phase 0 — measure the problem, build the enforcement

Two tools were built (both under `tools/novakai/`, mapped in `_tooling.mmd`):
...
- **FORCE** — `turn-gate.mjs`, a PreToolUse hook on `Read|Grep|Glob` (wired in
  ...
  batch the reads. A marker file (`.novakai-turn-gate.json`) is meant to grant one
...
Verify: `cat docs/novakai/turn-baseline.json` · `grep -n "turn-gate" .claude/settings.json`
...
Verify: `node --test tools/novakai/turn-gate.test.mjs tools/novakai/turns.test.mjs tools/novakai/metrics.test.mjs`
...
Verify: `npm run --silent novakai:metrics` (gate table, `turns` row) · protocol and
recorded outcome: `docs/novakai/SESSION_HANDOFF.md` (and its archive).
...
npm run novakai:turns -- check --file ~/.claude/projects/-Users-christopherdasca-Programming-novakai/<session-id>.jsonl
...
npm run novakai:turns -- summary --dir /tmp/postgate
# compare the medians against the "baseline" block in docs/novakai/turn-baseline.json,
...
This is the novakai philosophy applied to the agents themselves: measure first (with
