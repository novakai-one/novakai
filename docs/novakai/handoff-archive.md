...
> computed (`npm run novakai:status`, `npm run novakai:roadmap`); still-live
> sharp edges were promoted to `docs/novakai/KNOWN_EDGES.md` at rotation time.

## 0·prev·m10-gate-residuals (2026-07-04, session 6) — M10 residuals closed on `m10/gate-residuals`: bounce fixed (frozen-window grace), sidechains BIND (two stacked root causes found live), non-blocking reminder hooks live; NEXT: merge PR, then M9 demo (Phase C postponed per Chris)
...
capture (`touch .novakai-gate-debug`): (a) sidechain payloads carry the MAIN transcript path —
...
(subagent-priority / batch-reads, rotating, every 2nd main-thread Bash, `NOVAKAI_REMINDER_EVERY`)
...
|---|---|---|
| bounce fixed + cadence pinned | `node --test tools/novakai/turn-gate.test.mjs` | 14 pass, incl. bounce-repro (allow-grace where the deny bounced) + defiant cadence deny,allow,allow,deny |
...
| sidechain deny observed live | `grep 'turn-gate.mjs' docs/novakai/metrics/session-log.jsonl \| grep '"agent":"ac2' \| tail -3` | deny → allow-after-deny → allow-grace at 2026-07-04T10:44, agent ac20153a728253a74 |
| reminder hook non-blocking by construction | `node --test tools/novakai/reminder-hook.test.mjs` | 9 pass, incl. schema-safety (only hookSpecificOutput/additionalContext, never a decision field) |
| reminder hook registered | `grep -n "reminder-hook" .claude/settings.json` | PreToolUse matcher Bash |
| new tool mapped | `npm run novakai:tooling:verify` | green, reminder-hook node present |
| payload-capture knob | `grep -n "novakai-gate-debug" tools/novakai/turn-gate.mjs` | flag-file guarded, never affects decisions |
| sidechain root-cause recorded | `node -p "JSON.parse(require('fs').readFileSync('docs/novakai/turn-baseline.json','utf8')).validation.sidechainBinding"` | falsified → root-caused → fixed, dated |
...
`node --test tools/novakai/turn-gate.test.mjs tools/novakai/reminder-hook.test.mjs` → 23 pass, 0 fail.
...
(Paths above are pre-reorg — session 6 ran before PR3 moved `tools/novakai/*.mjs` into
`onboard/verify/plan/contract/status/gates/` sub-folders. Historical record, verbatim.)

## 0·prev·m10-phase-b (2026-07-04, session 5) — Phase B executed → Outcome W (unlisted): ALL gate denies were silently non-binding; fixed + proven binding live; subagent metering repaired; NEXT: merge this PR, then Phase C

Phase B ran per the session-4 protocol and found an outcome its X/Y/Z taxonomy didn't list.
**Outcome W:** every novakai gate (turn, edit, contract, plan) emitted stdout JSON
`decision:"deny"`; the current harness accepts only `approve|block`, fails schema validation,
and downgrades the deny to a non-blocking note — the tool call proceeds. No novakai gate had
...
| What | Verify it yourself | Expect |
|---|---|---|
| gates speak harness vocabulary | `grep -n "decision: 'block'" tools/novakai/*-gate.mjs` | 4 gates, 1 hit each |
| retry semantics pinned | `node --test tools/novakai/turn-gate.test.mjs` | 8 pass, incl. growing-transcript test |
| binding deny observed live | `grep '"gate":"turns"' docs/novakai/metrics/session-log.jsonl \| tail -5` | deny → allow-after-deny pairs at 2026-07-04T08:2x |
| hookTiming falsification recorded | `node -p "JSON.parse(require('fs').readFileSync('docs/novakai/turn-baseline.json','utf8')).validation.hookTiming"` | original claim + FALSIFIED 2026-07-04 |
| subagent tokens meter again | `npm run novakai:turns -- check --file ~/.claude/projects/-Users-christopherdasca-Programming-novakai/870eb983-2e01-478a-b206-3c5244de9ad1.jsonl --json` | subagentTokens ≥ 295208 (≥6 spawns) |
| phase narrative doc | `cat docs/novakai/turn-discipline.md` | phases 0/A/B/C, command-anchored, re-measure recipe |
...
- n=1 smoke after a real work session: `npm run novakai:turns -- check --file ~/.claude/projects/-Users-christopherdasca-Programming-novakai/<session>.jsonl` (exit 0 = targets met; caveat: batchRatio also penalizes legitimately serial Bash-heavy sessions — read the row, not just the exit code).
- Real verdict: copy ONLY post-merge transcripts (mtime after the PR merge — the earlier
  ...
  dir, then `npm run novakai:turns -- summary --dir <dir>`; compare medians vs the baseline
  block in docs/novakai/turn-baseline.json (working means: batchRatio ≥2.0 · cacheRead ≤~3.5M ·
...
**Carried:** M9 (W6) recorded demo per docs/novakai/demo/prep/recording-protocol.md.
...
tools/novakai/turn-gate.mjs) likely never passes on a live transcript — the denied call's
...
| What | Verify it yourself | Expect |
|---|---|---|
| tooling on main | `git log --oneline -5` + `ls tools/novakai/turn-gate.mjs` | 594bff8 merge in lineage; file exists |
| gate armed | `grep -n "turn-gate" .claude/settings.json` | PreToolUse matcher Read\|Grep\|Glob |
| mechanism (Phase A) | `node --test tools/novakai/turn-gate.test.mjs tools/novakai/turns.test.mjs tools/novakai/metrics.test.mjs` | 30/30 pass |
| full suites | `npm run spec:test:all` | green (336 pass, 0 fail) |
| measurement sane | `npm run --silent novakai:turns -- summary` | 19+ session rows; medians ≈ baseline block in docs/novakai/turn-baseline.json |
| dashboard | `npm run --silent novakai:metrics` | `turns` gate row (n/a until first live deny) + turn-discipline tail line |
...
2. Expect on the 4th: deny (`novakai turn-gate: 4 consecutive single-read turns…`); marker
   `.novakai-turn-gate.json` appears at repo root.
3. Re-issue the identical Read once, then read `npm run --silent novakai:metrics`:
   - **Outcome X** — denied again, `0 allow · N deny`, marker rewritten: retry defect CONFIRMED.
     Fix on a new branch: `marker.streak >= streak` → `<=` in tools/novakai/turn-gate.mjs; add a
...
5. Delete `.novakai-turn-gate.json` afterwards — MANDATORY: a lingering marker grants the next
...
- Real verdict: copy ONLY post-gate transcripts (mtime after 2026-07-04) to a scratch dir, then
  `npm run novakai:turns -- summary --dir <dir>`; compare medians vs the baseline block in
  docs/novakai/turn-baseline.json (working means: batchRatio ≥2.0 · cacheRead ≤~3.5M ·
  toFirstSrcEdit <50k continue-track · subagentTokens >0). Do NOT judge from the unscoped
...
## 0·prev·m10-measure-force (2026-07-04, session 3) — M10 turn-discipline: MEASURE (novakai:turns) + FORCE (turn-gate PreToolUse hook), baseline recorded
...
| What | Verify it yourself | Expect |
|---|---|---|
| MEASURE over real sessions | `npm run --silent novakai:turns -- summary` | per-session table + medians (batchRatio ~1.28, self-describing target lines) |
| FORCE hook wired | `grep -n "turn-gate" .claude/settings.json` | PreToolUse matcher Read\|Grep\|Glob |
| gate behavior proven | `node --test tools/novakai/turn-gate.test.mjs` | pass: deny at streak 4, one-free-retry marker, fail-open cases |
| one parser, no drift | `grep -l "lib/transcript.mjs" tools/novakai/turns.mjs tools/novakai/turn-gate.mjs` | both files |
| baseline + reassessment protocol | `cat docs/novakai/turn-baseline.json` | methodology, targets (batchRatio >=2.0, toFirstSrcEdit <50k), validation record |
| dashboard integrated | `npm run --silent novakai:metrics` | gate table gains `turns` row + turn-discipline tail line |
| tooling map complete | `npm run novakai:tooling:verify` | DONE (new modules mapped under co_metrics) |
...
docs/novakai/demo/prep/recording-protocol.md. New: review + merge `m10/turn-discipline`;
the gate goes live for the next session in this repo. After ~1 week of sessions, run the
reassessment in turn-baseline.json (`npm run novakai:turns -- summary` vs its baseline
...
(`id ufReviewPlan`) in unfold's io tab. (4) `docs/novakai/mvp-roadmap.json` — M0 manual
...
`docs/novakai/parity-checklist.md` — plan review row → `unfold-reachable (io tab →
...
| No signature drift | `npm run novakai:gate` | clean (spec and code in sync) |
| M0 built + new spine order | `npm run novakai:mvp` | M0 BUILT; spine `rename -> tooling-enforceable -> interface -> readability -> recorded-demo -> foreign-repo`; M9 listed before M7 |
| Ban intact | `npm run novakai:roadmap:audit` | both scans clean |
| Plan-review ruling recorded | `grep -n "2026-07-04/1" docs/novakai/parity-checklist.md` | hits |
...
| Map fresh | `npm run novakai:ship` | DONE line |
| Quiz pass bound to a live session | `npm run novakai:onboard` (STEP 4) | re-take in YOUR session — this session's pass never attests your read |
...
`npm run novakai:mvp` computes it all — never this file; queued: redesign
...
specs (`novakai:gate`). Per-module delta table + full pass list:
...
|---|---|---|
| Suite green | `npm run spec:test:all` | all pass |
| No exported-signature drift | `npm run novakai:gate` | clean |
...
| Map true + complete at HEAD | `npm run novakai:ship` | DONE line |
| Quiz pass bound to a live session | `npm run novakai:onboard` (STEP 4) | re-take in YOUR session |
...
`npm run novakai:mvp` computes it all, never this file.
...
`docs/novakai/plans/e4-f5-ci-predicates.plan.md`. Branch `e4-f5-ci-predicates` —
...
`NOVAKAI-CONTRACT:<id>` spawn sentinel (G4 gate validated the packet at spawn); the
orchestrator verified every landing with `novakai:acceptance` + `novakai:verify-change` and
...
`docs/novakai/probes/m5-tabs2-verbs.probe.js` — usage in its header). Branch
`m5-p-tabs2-a-verbs-build` — Chris reviews and merges. Never commit on `main` — standing
verdict in KNOWN_EDGES.md.

| What | Verify it yourself | Expect |
|---|---|---|
| P-tabs2 contract green | `npm run novakai:acceptance -- --plan docs/novakai/plans/m5-p-tabs2.plan.json` | 6/6 green — behavioural contract satisfied |
| A-verbs contract green | `npm run novakai:acceptance -- --plan docs/novakai/plans/m5-a-verbs.plan.json` | 13/13 green — behavioural contract satisfied |
| A-verbs fully landed | `npm run novakai:status -- --plan docs/novakai/plans/m5-a-verbs.plan.json` | 6 built · 0 pending · 0 drifted |
| P-tabs2 landed (then superseded) | `npm run novakai:status -- --plan docs/novakai/plans/m5-p-tabs2.plan.json` | 4 built + `uf-dock-tabs2` DRIFTED — expected supersession, NOT a regression: a-verbs deliberately widened `initUnfold` (see KNOWN_EDGES, cumulative-plans flavour); BUILT state verifiable at commit 9bb8597 |
| Pure fns PROVEN, not just shaped | `npm run novakai:verify-change -- --change uf-slice-targets --plan docs/novakai/plans/m5-p-tabs2.plan.json --json` (and `--change uf-verb-gate` on the a-verbs plan) | verdict `PASS`, behavioural proven:true (6/6 · 13/13) |
| Map true + complete at HEAD | `npm run novakai:ship` | DONE line; 0 unaccounted edges |
| M5 predicates | `npm run novakai:mvp` | M5 11/11 machine predicates (◐ only from its standing declared-manual line) |
| Types clean | `npx tsc --noEmit` | exit 0, no output |
| Runtime criteria (16) | `node docs/novakai/probes/m5-tabs2-verbs.probe.js` (needs playwright + dev server — header explains) | 16 `[PASS]` lines, `FINAL CONSOLE ERRORS (0)` |
| Ban intact | `npm run novakai:roadmap:audit` | both scans ✓ |
| Quiz pass bound to a live session | `npm run novakai:onboard` (STEP 4) | re-take in YOUR session — this session's pass never attests your read |
...
- `uf-dock-tabs2` reads DRIFTED on the p-tabs2 plan because a-verbs superseded the
  ...
  NOT catch such a drift (only `novakai:status`/`verify-change` do).
...
ruling whenever Chris rules. `npm run novakai:mvp` computes it all — never this file.
...
`docs/novakai/plans/m5-p-panel.plan.json`): the pure `ufDockReduce`
...
implemented per Chris's design (`docs/novakai/plans/m5-p-wires.plan.json`): the pure
...
| What | Verify it yourself | Expect |
|---|---|---|
| Plan fully landed (4 changes) | `npm run novakai:status -- --plan docs/novakai/plans/m5-p-wires.plan.json` | 4 built · "Plan fully landed" |
| Behavioural contract green (was 0/11 red) | `npm run novakai:acceptance -- --plan docs/novakai/plans/m5-p-wires.plan.json` | 11/11 green, exit 0 |
| Plan still author-coherent post-landing | `npm run novakai:plan-check -- --plan docs/novakai/plans/m5-p-wires.plan.json` | coherent (the landed add was flipped to modify per KNOWN_EDGES) |
| M5 per-feature predicates | `npm run novakai:mvp` | `M5` shows (4/4); manual note remains (more rows to drain) |
| Map re-synced, gate + edges green | `npm run novakai:ship` | DONE line; 0 unaccounted edges |
| Full tooling suite | `npm run spec:test:all` | exit 0, no failures |
| Full CI-equivalent chain | `npm run novakai:verify:full` | DONE line |
| Ban holds on all docs | `npm run novakai:roadmap:audit` | both scans ✓ |
| Runtime (browser): no wire crosses a foreign container; no default arrowheads; badges; group/leaf/wire selection rules | boot `npm run dev`, load `docs/novakai/_bundle.mmd` via the mermaid tab, expand groups, then select a group header / a leaf via the tree / click a badge | 0 crossings; arrows only after leaf-select or badge-click; inspector lists the exploded wire's carries |
...
`npm run novakai:onboard`, then `npm run novakai:mvp` for the computed M5 state; the parity
checklist rows are the feature enumeration. `npm run novakai:mvp` computes it all — never
this file.
...
The approved boot-flip plan (`docs/novakai/plans/m5-boot-flip.plan.json`, PR #33 review)
...
written: acceptance was red, the plan awaited Chris's review in the novakai app.
...
M4 executed through the full loop (design contract `docs/novakai/m4-read-primary-design.md` committed
...
under `SURFACE_KEY` (`novakai.surface.v1`, a config key string, allowlisted like `LS_KEY`). Branch
`m4-read-primary`, commits doc `458068c` → map+plan `f176bde` → code `cba5ee0`. Each row runnable.

| What | Verify it yourself | Expect |
|---|---|---|
| **M4 is BUILT — computed, not prose** | `npm run novakai:mvp` | `M4 — Read (unfold) migrated to main app (6/6)` [BUILT], zero manual lines |
| The behavioural contract was red before the code | `npm run novakai:acceptance -- --plan docs/novakai/plans/m4-read-primary.plan.json` | 10/10 green (strict normalize ×5, boot decision table ×5) |
| Plan coherent, fully landed | `npm run novakai:plan-check -- --plan docs/novakai/plans/m4-read-primary.plan.json` · `npm run novakai:status -- --plan docs/novakai/plans/m4-read-primary.plan.json` | coherent (6 changes, 7 deps) · 6 built |
...
| The new contract is mapped | `grep -c 'viewspec__AppSurface\|viewspec__normalizeSurface\|viewspec__resolveBootSurface' docs/novakai/_bundle.mmd` · same for `unfold__ufOpen\|unfold__ufClose` | 25 · 21 |
...
| Map true + complete + in sync | `npm run novakai:ship` → `git diff --stat docs/novakai/_bundle.mmd` | DONE line · empty |
| Status ban holds on all docs | `npm run novakai:roadmap:audit` | both scans ✓ |
| Run it | `npm run dev` → clear localStorage → reload | arrival = the reading overlay (5 region cards); ✕ → editor, reload stays editor; Read → reload stays read; `novakai.surface.v1` holds `'read'`/`'edit'` |
...
`npm run novakai:roadmap` / `npm run novakai:mvp` compute all of it — never this file.
...
M2b executed per the reviewed design contract (`docs/novakai/m2b-metrics-design.md`, §10 test-first
order, every step red before its code). One fail-silent emitter (`tools/novakai/lib/metrics-log.mjs`,
the one invariant: logging may never change a gate's decision, exit code, stdout or latency class),
one CLI (`tools/novakai/metrics.mjs` — `summary` + the transparent `wrap` ship recorder), and
instrumentation in ALL FOUR gates + `quiz.mjs check` + `verify-change.mjs` + `plan-cert.mjs` (CLI
path only — `certifyPlan` stays a pure import). The log (`docs/novakai/metrics/session-log.jsonl`)
is gitignored session telemetry; the summarizer is green on a fresh clone. Each row runnable.

| What | Verify it yourself | Expect |
|---|---|---|
| **M2b is BUILT — computed, not prose** | `npm run novakai:mvp` | `M2b — Compliance metrics (7/7)` [BUILT], zero manual lines |
| Emitter invariant proven (fail-silent on a broken destination, NOVAKAI_ROOT seam) | `node --test tools/novakai/lib/metrics-log.test.mjs` | 4/4 |
| Summarizer failure semantics (absent/empty log exit 0, malformed skip+count, 0/0 = n/a never 0%, `wrap` exit/signal transparency, start/end pairing, `--since`/`--last`) | `node --test tools/novakai/metrics.test.mjs` | 12/12 |
| Every instrumented tool metered with exit codes UNCHANGED (each file carries an M2b test) | `node --test tools/novakai/edit-gate.test.mjs tools/novakai/plan-gate.test.mjs tools/novakai/ship-staleness.test.mjs tools/novakai/contract-gate.test.mjs tools/novakai/quiz.test.mjs tools/novakai/verify-change.test.mjs` | 53/53 |
| plan-cert records on the CLI path only (library importers never double-record) | `node --test tools/novakai/plan-cert.test.mjs` | 4/4 (incl. the M2b wiring test) |
| Ship runs are recorded via a transparent wrapper | `grep -n 'metrics.mjs wrap --event ship' package.json` · `npm run novakai:ship; echo $?` · `npm run novakai:metrics` | `novakai:ship` wraps `novakai:ship:steps` · DONE + 0 · `ship runs : ≥1 run(s)` |
| The four intent metrics read back | `npm run novakai:metrics` (or `-- --json`) | quiz pass rate · per-gate allow/deny · ship runs/ok/aborted/median · cert pass rate · PASS_UNPROVEN ratio (n/a where no data) |
| The log is session-local, never committed | `git check-ignore docs/novakai/metrics/session-log.jsonl` | path echoed (ignored) |
| Determinism untouched: verify-change stdout byte-identical around the side log | `node --test tools/novakai/verify-change.test.mjs` · `npm run novakai:mutate` | 9/9 (incl. byte-identity test) · 4/4 HARNESS GREEN |
| Tooling self-map carries both new modules (I1) | `grep -c 'metrics' docs/novakai/_tooling.mmd` · `npm run novakai:tooling:verify` | ≥8 · DONE line |
| Whole suite green · typecheck clean | `npm run spec:test:all` · `npm run typecheck` | 298 pass 0 fail (285+6+2+5) · exit 0 |
| Src map untouched by this session (tools-only change) | `npm run novakai:ship` → `git diff --stat docs/novakai/_bundle.mmd` | DONE (512 nodes · 334 edges) · empty |
| Status ban holds on all docs | `npm run novakai:roadmap:audit` | both scans ✓ |

**Honest boundaries (do not oversell):**
- The emitter is fail-silent BY DESIGN (design §3/§8): a broken emitter silently undercounts rather
  than ever influencing a gate. `NOVAKAI_METRICS_DEBUG=1` surfaces emit errors.
- Test suites route emitter output to scratch sinks via `NOVAKAI_ROOT` (the seam design §3 names) —
...
open gaps; `npm run novakai:roadmap` / `npm run novakai:mvp` compute all of it — never this file.
...
contract committed first: `docs/novakai/m3-viewspec-design.md`). Each row runnable.

| What | Verify it yourself | Expect |
|---|---|---|
| **M3 is BUILT — computed, not prose** | `npm run novakai:mvp` | `M3 — ViewSpec contract (7/7)` [BUILT], zero manual lines |
| The behavioural contract is real (was red pre-impl) | `npm run novakai:acceptance -- --plan docs/novakai/plans/m3-viewspec.plan.json` | 7/7 green (migration, id-confinement, fold/reveal/guard/exclusion invariants) |
...
| Plan coherent, fully landed | `npm run novakai:plan-check -- --plan docs/novakai/plans/m3-viewspec.plan.json` · `npm run novakai:status -- --plan docs/novakai/plans/m3-viewspec.plan.json` | coherent (6 changes, 5 deps) · 6 built |
| Map true + complete + in sync (viewspec module + ufCommit are nodes) | `npm run novakai:ship` · `grep -c 'viewspec__' docs/novakai/_bundle.mmd` | DONE (512 nodes · 334 edges, 0 unaccounted) · ≥40 |
| Whole suite green · typecheck clean | `npm run spec:test:all` · `npm run typecheck` | 275 pass 0 fail (262+6+2+5) · exit 0 |
| Design contract + status ban | `ls docs/novakai/m3-viewspec-design.md` · `npm run novakai:roadmap:audit` | present · both scans ✓ |
...
(design contract `docs/novakai/m2b-metrics-design.md` §10, still the open P1 item) or **M4** (unfold →
main-app surface, now unblocked and ViewSpec-driven by construction); `npm run novakai:mvp` computes
...
contract `docs/novakai/m2b-metrics-design.md` and converts M2b's roadmap `manual` note into 7 machine
...
| What | Verify it yourself | Expect |
|---|---|---|
| The design contract exists | `ls docs/novakai/m2b-metrics-design.md` | present |
| M2b is computed against real predicates — the manual note is gone | `npm run novakai:mvp` | `M2b — Compliance metrics (1/7)`, six `✗ unmet` build predicates, zero `· manual` lines |
| The predicates prove instrumentation, not mere file existence | `grep -A9 '"id": "M2b"' docs/novakai/mvp-roadmap.json` | file ×2 · grep ×4 (`lib/metrics-log` call-sites in edit-gate + quiz, npm wiring, ship wrap) · cmd ×1 (summarizer green on empty log) |
| The design doc passes the status-marker ban | `npm run novakai:roadmap:audit` | both scans ✓ (16 docs) |
| Nothing yet implies build progress | `ls tools/novakai/metrics.mjs docs/novakai/metrics 2>&1` | both: No such file or directory |
| Map untouched by this session | `npm run novakai:ship` → `git diff --stat docs/novakai/_bundle.mmd` | DONE line · empty |

**Next (Scenario 1):** Chris reviews the design PR. On approval, the build session follows the
test-first order in `docs/novakai/m2b-metrics-design.md` §10; `npm run novakai:mvp` computes M2b's
...
| What | Verify it yourself | Expect |
|---|---|---|
| The hook is wired | `grep -n 'Edit\|Write' .claude/settings.json` | matcher `Edit\|Write` → `node tools/novakai/edit-gate.mjs` |
| Deny/allow logic proven offline | `node --test tools/novakai/edit-gate.test.mjs` | 8/8 |
| A src/ edit in a checkout with no quiz pass DENIES | `printf '{"tool_name":"Edit","tool_input":{"file_path":"src/main.ts"}}' \| NOVAKAI_ROOT=$(mktemp -d) node tools/novakai/edit-gate.mjs; echo $?` | deny JSON + `2` |
| The gate is in the tooling self-map (I1) | `grep -c 'editGate' docs/novakai/_tooling.mmd` | ≥4 (node + kind + src + meta) |
| Its test is in the ONE canonical suite (CI runs it by construction, F-06) | `grep -c 'edit-gate.test.mjs' package.json` | 1 |
| M2 progress is computed, not prose | `npm run novakai:mvp` | M2 (2/3): `Edit\|Write` + `ExitPlanMode` met · `ship` unmet |

**Hook 2/3 — ExitPlanMode plan-gate (this PR).** C3 coherence enforced at plan-approval time: exiting
plan mode DENIES while the named (`NOVAKAI-PLAN:<path>` in the plan text) or in-flight (`public/plan.json`)
...
| What | Verify it yourself | Expect |
|---|---|---|
| The hook is wired | `grep -n 'ExitPlanMode' .claude/settings.json` | matcher → `node tools/novakai/plan-gate.mjs` |
| Deny/allow logic proven offline | `node --test tools/novakai/plan-gate.test.mjs` | 9/9 |
...
| The gate is in the tooling self-map (I1) | `grep -c 'planGate' docs/novakai/_tooling.mmd` | ≥4 |
...
`novakai:ship`, it does not run the pipeline itself — a Stop hook that silently regenerated the map
...
| What | Verify it yourself | Expect |
|---|---|---|
| The hook is wired | `grep -n 'ship-staleness' .claude/settings.json` | Stop hook → `node tools/novakai/ship-staleness.mjs` |
| Deny/allow logic proven offline | `node --test tools/novakai/ship-staleness.test.mjs` | 7/7 |
| The gate is in the tooling self-map (I1) | `grep -c 'shipStaleness' docs/novakai/_tooling.mmd` | ≥4 |
| Its test is in the ONE canonical suite | `grep -c 'ship-staleness.test.mjs' package.json` | 1 |
| **M2 is BUILT — computed, not prose** | `npm run novakai:mvp` | `M2 — Protocol hooks live (3/3)` [BUILT] |
...
## 0·prev·m0 (2026-07-03, this session) — M0: repo renamed novakai → novakai · AUD5 register CLOSED by human confirmation
...
deliberately unchanged: `novakai:*` scripts, `novakai-spec-tools`, and the `@novakai-one/novakai-spec-tools`
...
| No stale repo refs remain (the one hit is the npm-scope tool-branding example, kept by decision) | `grep -rn "novakai-one/novakai" --exclude-dir=node_modules --exclude-dir=.git .` | exactly 1 hit: `tools/DISTRIBUTION.md` (`@novakai-one/novakai-spec-tools`) |
| Package renamed | `grep '"name"' package.json` | `"name": "novakai"` |
| AUD5 manual note removed → register closure is computed, not prose | `npm run novakai:audit` | AUD5 **[BUILT] (20/20)** — no manual line; AUD2 stays [PARTIAL] (its own sign-off, by design) |
| Required checks still bind post-rename | `curl -s https://api.github.com/repos/novakai-one/novakai/rules/branches/main` | ruleset with `required_status_checks` = buildspec-tests + novakai-drift |
| **Binding demonstrated live**: PR #22's first push went RED — `mutate.test.mjs`'s synthetic find-anchor was `"name": "novakai",`, which the rename removed from `package.json` (invisible locally: the harness worktrees from HEAD, which pre-commit still said novakai). Anchor updated to `"name": "novakai",` | `node --test tools/novakai/mutate.test.mjs` · `curl -s "https://api.github.com/repos/novakai-one/novakai/actions/runs?branch=m0-repo-rename&per_page=4"` | 5/5 · first-push runs `failure` (buildspec-tests), fix-push runs `success` |
...
| What | Verify it yourself | Expect |
|---|---|---|
| **F-19 fix verified by machine** — ruleset on `main` requires exactly the two audit-recommended checks | `curl -s https://api.github.com/repos/novakai-one/novakai/rules/branches/main` | ruleset 18426727: `required_status_checks` = **buildspec-tests + novakai-drift**, `pull_request` required, `non_fast_forward` + `deletion` blocked |
...
| (live bug) | replay.test dash-portability, `033b014` | `node --test tools/novakai/replay.test.mjs` → 5/5 |
| **F-04** | hollow predicates killed: `file` checks require `minBytes` (default 1 — a 0-byte file never reads BUILT again), `grep` checks take `count` (a lone pasted token no longer satisfies a table predicate); the audit's own predicates strengthened to content-bearing counts; `roadmap.mjs` gets its first test — 10 CLI-spawned deny fixtures, **3 failing pre-fix** (the exact A5/M3 attacks), 10/10 post-fix — wired into `spec:test:all` + CI; AUD5's check is now cmd+manual (computed) | `node --test tools/novakai/roadmap.test.mjs` → 10/10 · `npm run novakai:audit` → AUD5 [PARTIAL] (1/1 + manual) · `npm run novakai:plan-check -- --plan docs/novakai/plans/aud5-f04.plan.json` → coherent |
| **F-01** | contract-gate fails CLOSED on everything sentinel-shaped or unverifiable: near-miss sentinels (`NOVAKAI_CONTRACT`, wrong case, missing id) DENY; malformed stdin DENIES (the `Agent\|Task` matcher guarantees the payload is a spawn); prose "novakai contract" stays allowed; header rewritten to honest scope; the tests that LOCKED IN fail-open (T3) flipped — **4 new DENY tests red pre-fix**, 10/10 post-fix; primary deny branch pinned by reason text (closes the M2a masked-mutant redundancy) | `node --test tools/novakai/contract-gate.test.mjs` → 10/10 · AUD2 A1 repros now: typo → 2, malformed → 2, no-sentinel → 0 · plan: `docs/novakai/plans/aud5-f01.plan.json` |
| **F-02** | F4 gate fails CLOSED: shallow clone → exit 1 (the vacuous-CI hole — `novakai-drift` now checks out with `fetch-depth: 0`, per Chris), git-unavailable → exit 1 (was vacuous pass, A3), dirty-handoff bypass LOCAL-only (ignored under CI); `--check` gets its first CLI deny tests in date-pinned fixture repos — **3 fail-closed cases red pre-fix**, 10/10 post-fix; the accepted boundary (timestamp freshness, atomic-commit tie passes, H5 counters empty touches) is stated in the tool header | `node --test tools/novakai/handoff-fresh.test.mjs` → 10/10 · `grep -B1 'fetch-depth' .github/workflows/spec-gate.yml` → `novakai-drift` full-history (the job that runs `handoff:check`; `buildspec-tests` uses fixture repos and needs none) · plan: `docs/novakai/plans/aud5-f02.plan.json` |
| **F-03** | the quiz pass is now a machine-checked artifact: 100% `check` writes `.novakai-quiz-pass.json` bound to the sha256 of the exact map bytes; new `quiz verify` proves it against the CURRENT map (any map change → STALE); `onboard` STEP 4 prints the live state every session start; answers + pass artifact gitignored (A4 replay surface closed; same-map answer replay is inherent and stated in the header); 5 CLI deny/pass tests on a hand-knowable fixture map | `node --test tools/novakai/quiz.test.mjs` → 5/5 · `node tools/novakai/quiz.mjs verify` → states pass/stale/absent · plan: `docs/novakai/plans/aud5-f03.plan.json` |
| **F-05** | status-marker ban broadened from 2 literal regexes on 1 file to phrasing classes on `CLAUDE.md` + all of `docs/**`: status table cells (`\| done ✅ \|`), status sentences (`Status — shipped`), `state:` anywhere in a line (incl. HTML) all DENY; quoted context (fenced blocks, inline code, blockquotes) is exempt — the A6 false-positive on docs that *describe* the ban is gone; new `--audit-tree docs --allow docs/novakai/status-ban-allowlist.txt` (allowlist currently empty, entries need a reason); **3 tests red pre-fix**, 14/14 post-fix | `node --test tools/novakai/roadmap.test.mjs` → 14/14 · `npm run novakai:roadmap:audit` → both scans ✓ · plan: `docs/novakai/plans/aud5-f05.plan.json` |

| **F-06** | CI and local now consume ONE canonical gate list: `spec-gate.yml` runs `npm run spec:test:all` (the package.json list is the single source; the formerly local-only `slice-core` + both `novakai-lint` tests + `tooling-map` test now run in CI by construction) and keeps the real-plan acceptance step; new `novakai:verify:full` chains the five formerly-CI-only gates (roadmap:audit, cert, plan-check, acceptance, handoff:check) after `novakai:verify` for local parity; `gate-parity.test.mjs` — inside the suite — fails if a CI-only enumeration reappears or verify:full loses a gate; **2 tests red pre-fix**, 5/5 post | `node --test tools/novakai/gate-parity.test.mjs` → 5/5 · `npm run novakai:verify:full` → DONE banner · plan: `docs/novakai/plans/aud5-f06.plan.json` |

| **F-07** | `spec-gate.yml` triggers carry NO path filter any more — the old `paths:` list excluded `.claude/**` (the hooks), `public/plan.json` (the exact file cert/plan-check/acceptance target), `.quiz-answers.json` and root configs, so commits touching only those never ran the gate; every push/PR now gates, and a gate-parity test fails if a filter reappears; **1 test red pre-fix**, 6/6 post | `node --test tools/novakai/gate-parity.test.mjs` → 6/6 · `grep -c 'paths:' .github/workflows/spec-gate.yml` → 0 · plan: `docs/novakai/plans/aud5-f07.plan.json` |

| **F-08** | the tooling-map chain's promised deny paths are exercised for the first time (AUD3 T5 was ALLOW-only): an UNMAPPED load-bearing module → exit 1, a DANGLING `%% src` → exit 1, an unresolvable `#symbol` → exit 1 — three CLI fixtures in `tooling-map.test.mjs` asserting the exact problem class in stdout; the CI half of the finding was closed by F-06 (spec:test:all contains the tooling-map suite, so the whole chain now runs in CI by construction) | `node --test tools/novakai/tooling-map.test.mjs` → 8/8 · `grep -c 'spec:test:all' .github/workflows/spec-gate.yml` → ≥1 · plan: `docs/novakai/plans/aud5-f08.plan.json` |

| **F-09** | handoff staleness surfaces at session START, not only at clean Stop (attack A8: a crashed session never fires the Stop-hook nudge): `onboard` STEP 7 runs `handoff-fresh --check` and prints ✓ HANDOFF FRESH or ⚠ HANDOFF LAGS THE CODE + treat-as-suspect instructions — a nudge, not a gate (onboard's exit stays about map trust; F4 CI is the backstop); `onboard.test.mjs` (new, in the suite) is onboard's first test — **1 assertion red pre-fix** | `node --test tools/novakai/onboard.test.mjs` → 2/2 · `npm run novakai:onboard` → STEP 7 shows the live verdict · plan: `docs/novakai/plans/aud5-f09.plan.json` |

| **F-10** | the mutation-blind gate CLIs get their first spawn tests (AUD3 T4: `gate.mjs` was spawned by zero tests; plan-check / plan-cert / novakai-lint were fn-only): `cli-wiring.test.mjs` spawns each real CLI — gate in-sync → 0 / drift → 1 / no args → 2; plan-check incoherent → 1 / unreadable → 2 / real plan → 0; plan-cert usage errors → 2 (good path already spawned by loop-e2e); novakai-lint flat-mirror → 1 / no path → 2 / real bundle → 0. `handoff-fresh` was covered by F-02 | `node --test tools/novakai/cli-wiring.test.mjs` → 6/6 · plan: `docs/novakai/plans/aud5-f10.plan.json` |

| **F-11** | the A1 completeness pair and the grammar gate get their first deny fixtures (AUD3 T6: their exit-1 claims were proven only by running on good data): `coverage.mjs` uncovered file → 1 / covered → 0; `exports-coverage.mjs` hidden export → 1 / allowlisted → 0 (real ts-morph fixture project); `validate.mjs` edge-to-undefined-node → 1 / valid → 0 / no arg → 2 — all via the real spawned CLIs in `completeness.test.mjs` (in the suite) | `node --test tools/novakai/completeness.test.mjs` → 3/3 · plan: `docs/novakai/plans/aud5-f11.plan.json` |

| **F-12** | the approval-artifact emitter gets its rejection paths (AUD3 T7 was ALLOW-only): missing args → 2, unreadable plan → 2, all-changes-rejected → exit 0 with an EXPLICITLY empty artifact (plan.json 0 changes, clean checklist); plus a real behavior fix — `--accepted-only` on a plan with NO `verdicts` map used to silently export EVERY change (the opposite of the flag's promise) and now REFUSES with exit 2, emitting nothing; **1 test red pre-fix**, 7/7 post | `node --test tools/novakai/approve-export.test.mjs` → 7/7 · plan: `docs/novakai/plans/aud5-f12.plan.json` |

| **F-13** | the loop is proven to STOP, not just run (AUD3 T8: `loop-e2e` was a pure happy-path spine): a second chain feeds an incoherent fixture plan (dangling dep + modify of a nonexistent node) through the same stage order an orchestrating agent uses — plan-check blocks with exit 1, nothing downstream executes, no approval artifact is emitted | `node --test tools/novakai/loop-e2e.test.mjs` → 2/2 · plan: `docs/novakai/plans/aud5-f13.plan.json` |

| **F-14** | `orchestrate`’s only blocking check was data-dependent on the live `public/plan.json` (if that plan ever became fully built, the exit-1 path would go unexercised): a fixture plan adding a node whose symbol can never exist now proves exit 1 UNCONDITIONALLY — dispatched in wave 0, verdict FAIL, summary.fail 1, exit 1 | `node --test tools/novakai/orchestrate.test.mjs` → 7/7 · plan: `docs/novakai/plans/aud5-f14.plan.json` |

| **F-15** | A3 "parsers **provably** agree" can no longer vacuously pass: in CI (`CI=true`, set by GitHub Actions) an unavailable app-parser subprocess FAILS `parser-conformance.test.mjs` instead of silently `test.skip`ing the whole cross-parser half; locally the lenient skip stays (older Node). Testable via the `NOVAKAI_FORCE_APP_UNAVAILABLE` seam; `conformance-strict.test.mjs` (in the suite) locks both modes | `node --test tools/buildspec/conformance-strict.test.mjs` → 2/2 · `npm run spec:conformance` → 23/23, 0 skipped · plan: `docs/novakai/plans/aud5-f15.plan.json` |
| (live flakes ×2) | **suite made race-free after F-06/F-09 wired heavy spawns into it.** (a) `onboard.test` reruns `novakai:verify`, which rewrote `_bundle.mmd` via shell redirect — a parallel `edge-verify.test` could read the torn file (`import-backed edges too few: 0`, PR #16): `novakai:bundle` + `novakai:tooling:bundle` now write `.tmp` then `mv` (atomic). (b) onboard STEP 6 executed roadmap `cmd` predicates, spawning gate tools — incl. `orchestrate` with real git worktrees — concurrently with the suite (its worktree tripped `orchestrate.test`'s cleanup assertion): `onboard.test` now spawns with `NOVAKAI_ROADMAP_SKIP_CMD=1` (cmds only downgrade, per roadmap.mjs; every other step runs real). Suite wall 36s → 18s | `grep -c "mv docs/novakai/_bundle.mmd.tmp" package.json` → 1 · `npm run novakai:bundle && git status --short docs/novakai/` → clean · `grep -n NOVAKAI_ROADMAP_SKIP_CMD tools/novakai/onboard.test.mjs` → env set |

| **F-16** | the three orphaned diff tests run again (AUD3 T9: wired into neither suite nor CI — and two of them CANNOT run under plain `node --test`, their TS imports need the documented runner): `diff.test.mjs` joined the `node --test` list; `diff-views.test.mjs` + `diff-roundtrip.test.mjs` run via `run-bundled-test.mjs` at the end of `spec:test:all`; a gate-parity test keeps all three wired | `npm run spec:test:all` → 228 + 6 + 2, 0 fail · `node --test tools/novakai/gate-parity.test.mjs` → 7/7 · plan: `docs/novakai/plans/aud5-f16.plan.json` |

| **F-17** | the two session-protocol entry gates get their deny/verdict tests (AUD3 T10): `onboard` is proven to REFUSE — a doctored isolated worktree (one fragment deleted, node_modules symlinked) exits 1 with "STOP — the map is NOT trustworthy" AND names the exact uncovered file (right-reason pinned); `status` verdict classes locked on fixture plans against the real code — unimplemented add → `pending`, structure-only modify of a real node → `built`, wrong proposed signature → `drifted`, all-built → exit 0, no plan → exit 2 | `node --test tools/novakai/onboard.test.mjs` → 3/3 · `node --test tools/novakai/status.test.mjs` → 2/2 · plan: `docs/novakai/plans/aud5-f17.plan.json` |

| **F-18** | `waves` exit-0-on-cycle is now DOCUMENTED as design (a caller reading only the exit code was silently proceeding on cyclic plans) and gated on demand: new `--strict` exits 1 when the plan has ≥ 1 dependency cycle (matching the `verify-change --strict` precedent), the report still carries the cyclic ids, cycle-free strict runs stay 0, and the human CYCLE line says how to make it blocking; **1 test red pre-fix**, 7/7 post | `node --test tools/novakai/waves.test.mjs` → 7/7 · plan: `docs/novakai/plans/aud5-f18.plan.json` |

| **mutate harness** (post-register, from the AUD3 MH findings) | `novakai:mutate` re-runs the AUD3 mutation experiment mechanically instead of by-hand-in-the-main-tree: corpus `tools/novakai/mutations.json` seeded with M1/M2a/M2b/M3, each applied in an ISOLATED git worktree at HEAD (node_modules symlinked), two tiers (fast = the entry's own test file · full = whole `spec:test:all`), find-anchor must match HEAD exactly once or the harness REFUSES (stale corpus, exit 2). Expectations set to measured reality: **all 4 caught on both tiers** — the AUD5 fixes (F-01/F-02/F-04) demonstrably kill the AUD3 survivors. An expected-caught that survives = a deny test silently died → exit 1. Harness itself deny-proven by meta-tests (green / broken-expectation / refusal via the real CLI) | `npm run novakai:mutate` → 4/4 HARNESS GREEN · `npm run novakai:mutate:full` → 4/4 (≈2 min) · `node --test tools/novakai/mutate.test.mjs` → 5/5 · plan: `docs/novakai/plans/mutate-harness.plan.json` |
...
With F-18, **the AUD5 register is CLOSED**: every agent-fixable finding (F-01…F-18) is fixed test-first and merged through required-check-green PRs; F-19 was fixed by Chris (branch protection). The `novakai:mutate` harness (per the audit's mutation findings) is BUILT and green — see its ledger row above.
...
Output: `docs/novakai/audit/04-findings.md` — 19 findings consolidating A1–A8 + T1–T10/M1–M3, each
...
| What | Verify it yourself | Expect |
|---|---|---|
| AUD4 predicates met | `npm run novakai:audit` | AUD0–AUD1 ✓ · AUD2 [PARTIAL] (manual sign-off, by design) · AUD3 ✓ · **AUD4 [BUILT] (2/2)** · AUD5 unverified (fixes not started) |
| Register is complete, not sampled | `grep -c '^| F-' docs/novakai/audit/04-findings.md` · `sed -n '/Coverage map/,/AUD5 ordering/p' docs/novakai/audit/04-findings.md` | 19 rows · every A1–A8 / T1–T10 / M source id maps to a finding (A2 = HELD, no finding) |
| A7 resolution recorded verbatim | `grep -n 'no branch protection' docs/novakai/audit/04-findings.md` | F-19 row + header note, attested by Chris 2026-07-02 |
| Register passes the status-marker ban | `node tools/novakai/roadmap.mjs --audit-doc docs/novakai/audit/04-findings.md` | ✓ no hand-written status |
| Map untouched by the audit | `npm run novakai:ship` → `git diff --stat docs/novakai/_bundle.mmd` | DONE line · empty |
...
11 gap · 3 hygiene. Full table + AUD5 ordering: `docs/novakai/audit/04-findings.md`.

**Next (Scenario 1 — AUD5, fixes via the standard loop):**
1. **F-19 is Chris's, one setting:** protect `main`, require `buildspec-tests` + `novakai-drift` as
   required status checks. Post-fix verify: `gh api repos/novakai-one/novakai/branches/main/protection`.
2. Then one plan per finding under `docs/novakai/plans/` in the register's recommended order
   ...
   `manual` check in `docs/novakai/audit/audit-roadmap.json` to `cmd` checks.

## 0·prev·aud3 (2026-07-02, this session) — AUD3 LANDED: deny-path matrix + mutation spot-check (4 of 6 audit phases)

Executed AUD3 per `docs/novakai/audit/WORK_ORDER.md` (onboard ✓, quiz 12/12 seed 1 ✓, plan approved
by Chris). Output: `docs/novakai/audit/03-tests.md` — every GATE script classified
...
| What | Verify it yourself | Expect |
|---|---|---|
| AUD3 predicates met | `npm run novakai:audit` | AUD0 ✓ · AUD1 ✓ · AUD2 [PARTIAL] (manual sign-off, by design) · **AUD3 [BUILT] (2/2)** |
| Matrix covers every gate + findings carry repros | `grep -c '^| ' docs/novakai/audit/03-tests.md` · `grep -c 'repro' docs/novakai/audit/03-tests.md` | 42 table rows · 8 repro blocks |
| Mutation verdicts recorded | `grep -c 'SURVIVED' docs/novakai/audit/03-tests.md` | 6 (M1 + M3 survived, each cited 3×) |
| Mutations were reverted (nothing mutated survives in the tree) | `npm run spec:test:all` · `git diff --stat tools/` | 166/166 · empty |
| Map untouched by the audit | `npm run novakai:ship` → `git diff --stat docs/novakai/_bundle.mmd` | DONE line · empty |
| New doc passes the status-marker ban | `node tools/novakai/roadmap.mjs --audit-doc docs/novakai/audit/03-tests.md` | ✓ no hand-written status |
...
- **M3 SURVIVED:** `roadmap.mjs` — the status computer, the `novakai:roadmap:audit` CI gate, and
...
Executed the first three tooling-audit phases per `docs/novakai/audit/WORK_ORDER.md`. Outputs are
the three predicate-checked docs; the audit found real gate weaknesses (recorded, NOT fixed — fixes
are AUD5). Each row runnable.

| What | Verify it yourself | Expect |
|---|---|---|
| AUD0/AUD1 BUILT, AUD2 predicates met | `npm run novakai:audit` | AUD0 (2/2) · AUD1 (2/2) · AUD2 [PARTIAL] (2/2 auto; PARTIAL = the manual "coverage complete" sign-off, by design) |
| Inventory: every claim has a stable CLM id | `grep -c 'CLM-' docs/novakai/audit/00-inventory.md` | 60+ (claim rows across CLAUDE.md, tool headers, echo strings) |
| Every claim classified once | `for n in $(grep -oE 'CLM-[0-9]+' docs/novakai/audit/00-inventory.md \| sort -u); do echo "$n $(grep -c "\| $n " docs/novakai/audit/01-claims.md)"; done` | every id → 1 |
| Attacks carry repros | `grep -c 'repro:' docs/novakai/audit/02-attacks.md` | 9 |
| Map still in sync (audit touched no code) | `npm run novakai:ship` | DONE line; `git diff --stat docs/novakai/_bundle.mmd` empty |

**Confirmed-BROKEN gates (repro in `02-attacks.md`), for AUD4/AUD5:**
- **contract-gate fail-open (A1):** sentinel typo `NOVAKAI_CONTRACT`, malformed/empty/absent stdin
  all exit 0. Only the exact-cased `NOVAKAI-CONTRACT:` + unresolvable-id path DENIES (exit 2). repro:
  `printf '{"tool_name":"Agent","tool_input":{"prompt":"NOVAKAI_CONTRACT: x"}}' | node tools/novakai/contract-gate.mjs; echo $?` → 0.
...
`node tools/novakai/roadmap.mjs --roadmap docs/novakai/audit/audit-roadmap.json`. Then AUD4 findings
...
Stage 4 of `docs/novakai/plans/unfold-ux-repair.md` executed per protocol (onboard ✓, quiz 12/12 ✓,
...
| Both are in the shipped map with gated signatures | `grep -c "%% src unfold__uf\(SelectGroup\|GroupConns\) " docs/novakai/_bundle.mmd` | 2 |
| Map true + complete + in sync | `npm run novakai:ship` | DONE line, 0 unaccounted (325 edges) |
...
| Stage log is the fine-grained record | `sed -n '/stage 4 · 53c32a6/p' docs/novakai/plans/unfold-ux-repair.md` | one line: verified behaviors + notes |
...
> plan supersedes P2–P4 ordering below — see `docs/novakai/plans/unfold-ux-repair.md` (8 issues,
...
| Plan reads BUILT after the documented add→modify flip (4th lifecycle occurrence) | `npm run novakai:status -- --plan docs/novakai/plans/unfold-integration.plan.json` | **7 built · 1 pending** (pending = `read-review-overlay`, POSTPONED by verdict) |
| Map true + complete + in sync | `npm run novakai:ship` | DONE line (498 nodes · 319 edges, 0 unaccounted) |
...
pipeline parser `tools/buildspec/mmd-parse.mjs` (parse + `toMmd` emit); bundler pass-through `tools/novakai/bundle.mjs`;
validation `tools/novakai/validate.mjs` (undeclared group / missing member / id-collision = ERROR); authored grouping in
`docs/novakai/root.mmd` (promoted verbatim from `sandbox/unfold/hierarchy.json`, which is now **deleted** per the approved intent).
...
| What | Verify it yourself | Expect |
|---|---|---|
| 6 wirings landed, 2 held for review | `npm run novakai:status -- --plan docs/novakai/plans/unfold-integration.plan.json` | **6 built · 2 pending** (pending = `read-review-overlay`, `grouping-directive`) |
| Plan coherent after the documented add→modify lifecycle flip | `npm run novakai:plan-check -- --plan docs/novakai/plans/unfold-integration.plan.json` | coherent (8 changes, 3 deps) |
...
| Every keystone is in the shipped map with a gated signature | `grep -c "%% src unfold__uf\(SelectSync\|PersistView\|TrustLayer\|WirePath\|RenameInPlace\|MountFrontmatter\) " docs/novakai/_bundle.mmd` | 6 |
| Map true + complete + in sync + edges accounted | `npm run novakai:ship` | DONE line (490 nodes · 311 edges, 0 unaccounted) |
...
| What | Verify it yourself | Expect |
|---|---|---|
| All 7 stage changes landed | `npm run novakai:status -- --plan docs/novakai/plans/unfold-v3-stage.plan.json` | **7 built · 0 pending** |
| Plan coherent after the documented add→modify lifecycle flip | `npm run novakai:plan-check -- --plan docs/novakai/plans/unfold-v3-stage.plan.json` | coherent (7 changes, 4 deps) |
...
| Map true + complete + in sync with the new symbols | `npm run novakai:ship` | DONE line (483 nodes · 300 edges, 0 unaccounted) |
| New nodes in the map with gated signatures | `grep -c "unfold__uf\(EnterStagger\|FocusDim\|Reframe\|StageMode\|Proxies\|Travel\|TypeFocus\)" docs/novakai/_bundle.mmd` | >0 (nodes + kind + src + fm:meta) |
...
**Next (Scenario 1):** `npm run novakai:status -- --plan docs/novakai/plans/unfold-integration.plan.json` — 8 pending feature-parity wirings, orthogonal to stage; `read-review-overlay` and `grouping-directive` are flagged **design-review-first** in their intents (do not build without human review).
...
| **Build plan** — 7 falsifiable adds, all `unfold` module keystones | `npm run novakai:plan-check -- --plan docs/novakai/plans/unfold-v3-stage.plan.json` | coherent (7 changes, 4 deps) |
| Resume point | `npm run novakai:status -- --plan docs/novakai/plans/unfold-v3-stage.plan.json` | 7 pending — phase 1 (stagger, focus-dim, reframe) is low-risk and unblocks phase 2 |
| Prior integration plan still valid underneath | `npm run novakai:status -- --plan docs/novakai/plans/unfold-integration.plan.json` | 8 pending (feature-parity wiring; orthogonal to stage) |
...
| **Reading mode landed in `src/`** — `initUnfold` (planner isolation pattern): full-screen folded view of ctx.state, 7 opt-in layers, browse tree, inspector with ctx.bodies source | `grep -n "initUnfold" src/main.ts` · `npm run novakai:gate` · open `/` → **Read** button | wired in composition root · in sync (unfold in `_bundle.mmd`) · 465-node map arrives as 40 folded cards |
| Map true+complete with the new module | `npm run novakai:ship` · `npm run spec:test:all` | DONE line (475 nodes · 293 edges, 0 unaccounted) · 158/158 |
| **The migration is a plan, not prose** — remaining feature-wiring enumerated as falsifiable adds | `npm run novakai:plan-check -- --plan docs/novakai/plans/unfold-integration.plan.json` · `npm run novakai:status -- --plan docs/novakai/plans/unfold-integration.plan.json` | coherent (8 changes) · **8 pending** — the build checklist for the next sessions |

**Where to resume (Scenario 1):** `npm run novakai:status -- --plan docs/novakai/plans/unfold-integration.plan.json`.
...
| Repo gates unaffected | `npm run novakai:gate` · `npm run novakai:roadmap:audit` | in sync · pass |
...
("clusters"); trust layer derives from `novakai:trust` semantics (signatures verified · desc advisory ·
edges code-backed unless in `docs/novakai/edge-advisory-allowlist.txt`); `hierarchy.json` stays a
...
`.mmd`, and is invisible to the novakai tooling — so it is orthogonal to the understand→…→re-sync
...
| Tooling still green (sandbox is outside `src/`) | `npm run novakai:gate` · `npm run novakai:exports` · `npm run novakai:coverage` | in sync · PASS · PASS |
...
contract, waves, orchestrate…) were invisible to it. They now have their own novakai —
`docs/novakai/_tooling.mmd`, a **sibling bundle** kept out of the ts-morph `src` gate — proven
true + complete + architectural + deterministic by machine. Each row is runnable.

| What | Verify it yourself | Expect |
|---|---|---|
| Tooling map bundles + valid + architectural + complete + deterministic | `npm run novakai:tooling:verify` | DONE line · lint PASS · all 32 modules mapped · 5/5 tests |
| Every load-bearing `tools/` module is mapped or audited-excluded (no silent gap) | `npm run novakai:tooling:coverage` | PASS — 32 modules mapped, 32 %% src resolve |
| It is architecture altitude, not a flat file-mirror | `node tools/novakai/novakai-lint.mjs docs/novakai/_tooling.mmd` | RESULT: PASS (0 warnings) |
| Exclusions are audited, not silent | `cat docs/novakai/tooling-curation-allowlist.txt` | 5 entries (scratch / harness / contract-instrument), each with a reason |
| The committed map is fresh (re-bundle is byte-identical) | `npm run novakai:tooling:bundle && git status --short docs/novakai/_tooling.mmd` | clean (no diff) |
| Phase I computes BUILT (status, not prose) | `npm run novakai:roadmap` | `I1 [BUILT]` (5/5) · 32 built |
| The `src` map + whole suite still green (the tooling map is a sibling) | `npm run novakai:verify` · `npm run spec:test:all` | 0 unaccounted edges · 158/158 |
| Review overlay that authored this (viewable in the editor) | `npm run novakai:plan-check -- --plan docs/novakai/plans/tooling.plan.json` | coherent (103 changes) |

**How it was built (the novakai way):** 6 subsystem fragments (`tools/**/<subsystem>.novakai.mmd`)
authored by 6 subagents, each under an enforceable, self-verifiable contract —
`node tools/novakai/frag-check.mjs <fragment> --container <id> --expect <ids>` must exit 0 — then
gated centrally by `novakai:tooling:verify`. No subagent output was accepted on prose; only the checks.

**New files:** `docs/novakai/root-tools.mmd`, `docs/novakai/_tooling.mmd`,
`docs/novakai/tooling-curation-allowlist.txt`, `docs/novakai/plans/tooling.plan.json`,
`tools/novakai/{tooling-coverage,frag-check,tooling-map.test}.mjs`, and 6 fragments
(`tools/buildspec/buildspec.novakai.mmd`, `tools/novakai/{integrity,understand,continuity,plan-approve,contract-spine}.novakai.mmd`).
**Edited:** `package.json` (scripts + suite), `docs/novakai/roadmap.json` (Phase I), `CLAUDE.md` (I1 def).

**Honest boundaries (do not oversell):**
- **Structure-only.** `.mjs` carries no `fm` signature and is NOT gated by `novakai:gate` (ts-morph
  `allowJs:false`). The tooling map's truth is `novakai:tooling:verify` (grammar + lint + completeness
...
- **Separate bundle.** `novakai:onboard` / `novakai:ship` still operate on `_bundle.mmd` (the app).
  The tooling map is enforced by `novakai:tooling:verify`, which runs inside `spec:test:all` (so CI
...
`node --test` + `novakai:replay`; status is computed by the roadmap predicate. Each row is runnable.
...
| **H2 — editor decision artifact**: the editor emits `approved-plan.json` = plan+verdicts; the CLI mints the bundle | `grep -n approved-plan.json src/panel/planner.ts` · `node --test tools/novakai/approve-export.test.mjs` | present · 4/4 (incl. editor→CLI round-trip) |
| **H3 — verify-change `--strict`**: a shaped-but-unproven change no longer exits 0 | `node tools/novakai/verify-change.mjs --change fit-clamp --json --strict; echo $?` · `… --change frame-transform --json --strict; echo $?` | `1` (unproven) · `0` (proven) |
| **H4 — orchestrator + worktree isolation**: waves → per-change worktree+contract → strict verdict → canonical summary | `node tools/novakai/orchestrate.mjs --plan public/plan.json` · `node --test tools/novakai/orchestrate.test.mjs` | wave-0 dispatch + summary (exit 1 = unbuilt, expected) · 6/6 |
| H4 determinism (worktree side-effects must not perturb stdout) | `node tools/novakai/replay.mjs --task "node tools/novakai/orchestrate.mjs --plan public/plan.json --json" --n 5` | `DETERMINISTIC` · one hash |
| **H5 — handoff content-falsifiability**: a handoff that misstates a file's git state fails the gate | `node --test tools/novakai/handoff-fresh.test.mjs` | 5/5 (incl. the real two-bullet pattern + no false positive) |
| Phase H computes BUILT (status, not prose) | `npm run novakai:roadmap` | H1–H5 `[BUILT]` · 32 built |
| Plan coherent + certified; whole suite green; map in sync | `npm run novakai:plan-check -- --plan docs/novakai/plans/phase-h.plan.json` · `npm run spec:test:all` · `npm run novakai:gate` | coherent · 158/158 · in sync |

**Plan + machine-checked intent** (coherence proven by `npm run novakai:plan-check -- --plan
docs/novakai/plans/phase-h.plan.json`): `docs/novakai/plans/phase-h.plan.json`.

**Honest boundaries (do not oversell):**
- Like Phase G, four of five are `.mjs` TOOLING — deliberately NOT in `_bundle.mmd`, NOT gated by
  `novakai:gate`; verified by `node --test` + `novakai:replay`. The plan is checked by
  `novakai:plan-check` only (it also passes `plan-cert` with zero delta). H2's editor change IS
  app-TS and rides `novakai:gate`.
...
- **H5 retains the dirty-handoff bypass**: while `SESSION_HANDOFF.md` is modified in the working
  tree, `novakai:handoff:check` exits 0 without running the content check (the agent is editing it).
  The content check's correctness is proven by `node --test tools/novakai/handoff-fresh.test.mjs`
...
| What | Verify it yourself | Expect |
|---|---|---|
| **G1 — contract packet**: self-contained + byte-deterministic, routes to real map/source/cone | `npm run novakai:contract -- --change frame-transform --json` | canonical packet: `source`, `signature`, `acceptance.cases`, `blastRadius`, `deps`, `contractHash` |
| **G2 — verdict is data-only** (no prose/paths/time), PASS on a real green change | `npm run novakai:verify-change -- --change frame-transform --json` | `"verdict":"PASS"` · behavioural 3/3 · `verdictHash` |
| G2 — verdict is **honest about strength** (3-valued) | `npm run novakai:verify-change -- --change fit-clamp --json` · `… --change frame-node --json` | `PASS_UNPROVEN` (built, no contract) · `FAIL` (pending) |
| **G3 — the 100→100 proof**: one hash across N runs | `npm run novakai:replay -- --task "node tools/novakai/verify-change.mjs --change frame-transform --json" --n 20` | `DETERMINISTIC — all 20 runs identical` · exit 0 |
| G3 — leak detector is **not vacuous** (catches a real leak) | `npm run novakai:replay -- --task "node -e \"process.stdout.write(String(Math.random()))\"" --n 8` | `NON-DETERMINISTIC` · exit 1 |
| **G4 — spawn-gate** enforces the contract, fails open | `node --test tools/novakai/contract-gate.test.mjs` · `grep -n contract-gate .claude/settings.json` | 6/6 · `PreToolUse` hook wired |
| **G5 — parallel execution scheduler**: deterministic topological waves from deps + live status | `npm run novakai:waves -- --plan public/plan.json` | `wave 0 (ready now): …` dispatchable set · later waves unlock as deps land |
| G5 — the waves output is deterministic (100→100) | `npm run novakai:replay -- --task "node tools/novakai/waves.mjs --plan public/plan.json --json" --n 12` | `DETERMINISTIC` · one hash · exit 0 |
| Phase G computes BUILT (status, not prose) | `npm run novakai:roadmap` | G1–G5 `[BUILT]` · 32 built |
| **E4 gap fixed** — acceptance now runs on the REAL plan in CI (was engine-test only) | `grep -n "novakai:acceptance -- --plan" .github/workflows/spec-gate.yml` | present in buildspec-tests |
| Map still true+complete (tooling lives outside src/) · whole suite green | `npm run novakai:verify` · `npm run spec:test:all` | green · all pass |

**New files (all in `tools/novakai/`):** `lib/canonical.mjs`, `contract.mjs`, `verify-change.mjs`,
...
`docs/novakai/plans/subagent-contract.plan.json`. **Edited:** `package.json` (scripts + suite),
`docs/novakai/roadmap.json` (Phase G), `.github/workflows/spec-gate.yml` (G tests + E4 fix),
...
- The spine is **.mjs tooling, not app TS symbols** — it is deliberately NOT in `_bundle.mmd` and NOT
  gated by `novakai:gate` (the gate runs ts-morph `allowJs:false`; recon confirmed). Its verification
  is its own `node --test` suites + `novakai:replay`. The meta plan
  (`docs/novakai/plans/subagent-contract.plan.json`) is checked by `novakai:plan-check` only.
...
(`novakai:waves`, G5) — an orchestrator can fan out the wave-0 set to N parallel subagents, each
under its `novakai:contract`, collect `novakai:verify-change` verdicts, then advance. What remains is
...
| What | Verify it yourself | Expect |
|---|---|---|
| **Map was incomplete:** `camera.zoomToNode` existed in code + was called cross-module, but was absent from the map. Now documented. | `grep -n camera__zoomToNode docs/novakai/_bundle.mmd` | node present (kind/sig/src + `--> applyCam` edge) |
| Fixed map is true + complete + in sync | `npm run novakai:ship` | DONE line · gate in sync · 0 unaccounted edges (285) |
| `zoomToNode` is a real cross-module call, not dead code | `grep -n "camera.zoomToNode" src/panel/navigator.ts` | navigator.ts:124 |
| In-flight plan state (unchanged by the fix — `frameNode` still absent) | `npm run novakai:status -- --plan public/plan.json` | 8 built · 8 pending |
| Plan is certified but **awaits human review** — 0 changes carry a signature | `node tools/novakai/plan-cert.mjs --plan public/plan.json` | CERTIFIED · "Safe to send to human review" |
...
**Open risk (the novakai gap this fault exposed):** inner / API-surface functions are not
...
| Behavioural contract GREEN (was red pre-impl) | `npm run novakai:acceptance -- --plan public/plan.json` | 3/3 green · "the change is DONE (shaped AND correct)" |
| Map in sync (signature gate) | `npm run novakai:gate` | `state__frameTransform` signature matches code |
| Change reads BUILT | `npm run novakai:status -- --plan public/plan.json` | `frame-transform [BUILT]` (modify) · 9 built · 8 pending |
| Plan coherent + cert green + full suite green | `npm run novakai:plan-check -- --plan public/plan.json` · `npm run spec:test:all` | coherent (17) · 158/158 |
...
| What | Verify it yourself | Expect |
|---|---|---|
| **A5 — edge verification.** The whole call graph was UNVERIFIED (283 edges, warnings only) — yet blast-radius / `downstreamCone` all walk it. Now every edge is code-backed or audited. | `npm run novakai:edges` | 281 code-backed · 4 advisory · **0 unaccounted** |
| A5 fails closed | `node --test tools/novakai/edge-verify.test.mjs` | 5/5 (strict exits 1 on an unaccounted edge) |
| A5 advisory edges are audited, not hidden | `cat docs/novakai/edge-advisory-allowlist.txt` | 4 `ctx.hooks` edges, each with rationale |
| **A6 — type gate tightened.** Object-literal + function types now compared, not skipped as prose. | `npm run novakai:gate` | ✓ in sync · **1** prose hole (was 32) |
| A6 locked + found real drift | `node --test tools/buildspec/normtype.test.mjs` | 25/25 (also fixed `showTab` `which` drift) |
| **C3 — authoring-time coherence.** | `npm run novakai:plan-check -- --plan public/plan.json` | ✓ coherent (16 changes) |
| **E1 — single approval export.** | `npm run novakai:approve -- --plan public/plan.json --out /tmp/x` | approved.mmd + contracts + CHECKLIST.md + plan.json |
...
| **F4 — meta-loop is verifiable.** Handoff must be ≥ as fresh as the last code commit. | `npm run novakai:handoff:check` | ✓ (exits 1 when the handoff lags code) |
| **F5 — the loop runs end-to-end** (plan-check → cert → approve → status → writeback → edges) on the real plan, as one chain. | `npm run novakai:loop` | 1/1 |
| **trust report reflects A5/A6** | `npm run novakai:trust` | ~2526 verified · 32 partial · 0 unverified edges |
| Whole computed roadmap | `npm run novakai:roadmap` | 32 built (Phase A–I) |
| Nothing regressed | `npm run spec:test:all` · `npm run typecheck` | 158/158 · clean |

New files: `tools/novakai/{edge-verify,plan-check,approve-export,handoff-fresh,loop-e2e.test,edge-verify.test,plan-check.test,approve-export.test}.mjs`,
`tools/buildspec/normtype.test.mjs`, `docs/novakai/edge-advisory-allowlist.txt`, `.claude/settings.json`.
...
`npm run novakai:verify` green ⟺ the map is structurally true + complete (A1) **and its edges
...
## 3. Remaining intent (run `npm run novakai:roadmap` for live status)
...
`docs/novakai/onboard-cost-design.md`; plan `docs/novakai/plans/onboard-cost.plan.json`;
...
Verify rows lived in the session entry; the durable ones: `node --test tools/novakai/quiz.test.mjs`
...
small src feature — unfold status readout (docs/novakai/demo/prep/feature-choice.md).
...
| #43 staleness predicate | `node tools/novakai/ship-staleness.mjs < /dev/null; echo $?` | 0 on clean tree; 2 after dirtying src/ |
| #43 stamp idempotent | run `npm run novakai:ship` twice → `git status --porcelain` | empty both times |
| #44 dead code gone | `ls src/panel/diff-workspace.ts` | no such file |
| #44 rulings recorded | `grep -n "2026-07-04/2" docs/novakai/parity-checklist.md` | deletion ruling hits |
| Demo plan coherent | `npm run novakai:plan-check -- --plan docs/novakai/demo/prep/plan.json` | ✓ coherent |
| Demo protocol | `cat docs/novakai/demo/prep/recording-protocol.md` | capture method, artifact set, M9 predicates |
| Suites on every PR branch | `npm run spec:test:all && npm run test:src && npm run build` | all pass |
| M9 next in spine | `npm run novakai:mvp` | M9 (P4) listed before M7 |
...
docs/novakai/demo/prep/recording-protocol.md — fresh 0-context agent runs the loop on the
status-readout request; Chris drives planner review (genuine mouse input only);
`novakai:verify-change` with `--strict`; artifacts land in docs/novakai/demo/ and M9's manual
