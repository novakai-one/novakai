...
> `docs/novakai/plans/m4-read-primary.plan.json`.
...
`SURFACE_KEY = 'novakai.surface.v1'` (a `config.ts` export next to `LS_KEY`/`PREF_KEY`, allowlisted
...
| key | owner | content |
|---|---|---|
| `novakai.surface.v1` | `config.ts#SURFACE_KEY` (allowlisted export) | `'read'` \| `'edit'` |
| `novakai.autosave.v1` | existing | model (unchanged) |
| `unfold.view` | existing | per-diagram ViewSpec (unchanged) |

## 7. Roadmap predicate conversion (M4 `manual` → machine)

The single `manual` note in `docs/novakai/mvp-roadmap.json` (M4) is replaced by:
...
2. Fragments: 3 viewspec nodes + 2 unfold nodes + `main__firstRender` desc → `novakai:bundle`
...
4. Author the plan; `novakai:plan-check` green; **`novakai:acceptance` red** (7 cases, symbols
   absent).
...
6. Convert the roadmap predicates (§7); `novakai:mvp` computes M4.
