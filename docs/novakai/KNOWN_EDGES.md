...
- `novakai:verify-change`'s `PASS_UNPROVEN` verdict exits 0 exactly like `PASS`. A caller
...
- Plan lifecycle gap (recurred 3×): once an `add` change lands, it must be hand-flipped
  to `modify` to keep `novakai:plan-check` coherent. A built-add→done transition is a
  ...
  `novakai:status` stays truthful ("node removed" = BUILT) and CI plan-check targets
...
- `novakai:orchestrate` (H4) is a v1 driver: it provisions per-change worktrees and
...
- Metrics log caveats (design: `docs/novakai/m2b-metrics-design.md` §3/§8/§11): the
  emitter is fail-silent by design (`NOVAKAI_METRICS_DEBUG=1` surfaces emit errors);
...
- The tooling self-map (`docs/novakai/_tooling.mmd`, I1) is module-level completeness,
  structure-only — `.mjs` carries no signature gate. Its truth is
  `npm run novakai:tooling:verify` plus each module's `node --test` suite, not the
...
- **Harness hook JSON vocabulary (observed live 2026-07-04, session 5):** a PreToolUse
  ...
  only a `hook_non_blocking_error` attachment records it. Every novakai gate emitted
  `deny` from birth, so **no gate had ever actually blocked live** before the 2026-07-04
  fix. Any new gate must emit `block`. Verify: `grep -n "decision: 'block'" tools/novakai/*-gate.mjs`
  → 4 gates.
- **Harness PreToolUse timing — MAIN THREAD (falsifies the original hookTiming
  ...
  `validation.hookTiming`; pinned by `node --test tools/novakai/turn-gate.test.mjs`.
- **Harness PreToolUse timing — SIDECHAINS are the OPPOSITE (live-fire 2026-07-04,
  ...
  genuine streak break). Verify: `node --test tools/novakai/turn-gate.test.mjs`.
- **Sidechain hook payload (captured live 2026-07-04, session 6):** PreToolUse fires for
  ...
  `isSidechain`. Payload capture knob: `touch .novakai-gate-debug` → turn-gate appends raw
  payloads + computed decision inputs to `.novakai-gate-debug.jsonl` (never affects
...
- **Non-blocking PreToolUse injection (live-fire 2026-07-04, session 6):**
  ...
  effect). Live example: `tools/novakai/reminder-hook.mjs` (throttle:
  `NOVAKAI_REMINDER_EVERY`, default 2).
- **Transcript format drift (2026-07-04):** subagent spend in session transcripts is now
  ...
  Verify: `node --test tools/novakai/turns.test.mjs`.
...
- 4 advisory edges are audited, not proven — real `ctx.hooks`/runtime relations with no
  ...
  non-normalizable). Verify the live split: `npm run novakai:trust`.
- The SessionStart/Stop protocol hooks live in `.claude/settings.json` and fire only in
  this harness; F4 in CI (`novakai:handoff:check`) is the verifying backstop.
- **Standing verdict (2026-07-04) — `ctx.hooks` members are not, and will not be, enumerated
  ...
  being real, wired hooks (verify: `grep -c "enterContainer\|plannerOpen" docs/novakai/_bundle.mmd`
  ...
  that type's signature (`novakai:gate`), same as any other exported type. Do NOT add per-hook
...
- A synthetic `dispatchEvent(new PointerEvent('pointerdown', …))` (no OS-backed pointer
  ...
  2026-07-03, see `docs/novakai/probes/m5-tabs2-verbs.probe.js` header). Drive probes
