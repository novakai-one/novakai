...
target (`npm run novakai:mvp` computes how much of it has landed — this doc never says).

The roadmap intent it serves, quoted from `docs/novakai/mvp-roadmap.json`:
...
`quiz.mjs check` overwrites a single pass marker (`.novakai-quiz-pass.json`) and a failed
attempt leaves no record; a `novakai:ship` run leaves only its regenerated artifacts. One green
...
- `tools/novakai/lib/metrics-log.mjs` — the emitter (one function, fail-silent).
- `tools/novakai/metrics.mjs` — the CLI: `summary` (the `npm run novakai:metrics` entry) and
...
One JSON object per line in `docs/novakai/metrics/session-log.jsonl`. Every line carries the
...
## 3. Emitter contract — `tools/novakai/lib/metrics-log.mjs`

One export:

```js
recordEvent(fields, root)   // root defaults to NOVAKAI_ROOT || repo root
// -> mkdirSync(docs/novakai/metrics, { recursive: true })
...
that cost is accepted and named here. `NOVAKAI_METRICS_DEBUG=1` prints emit errors to stderr
for development. `NOVAKAI_ROOT` is honored so the existing hermetic-fixture test pattern
...
| --- | --- | --- |
| `tools/novakai/edit-gate.mjs` | `gate` (`gate:"edit"`) | at each allow/deny exit path |
| `tools/novakai/plan-gate.mjs` | `gate` (`gate:"plan"`) | at each allow/deny exit path |
| `tools/novakai/ship-staleness.mjs` | `gate` (`gate:"ship-staleness"`) | at the block / fresh exit paths |
| `tools/novakai/contract-gate.mjs` | `gate` (`gate:"contract"`) | at each allow/deny exit path |
| `tools/novakai/quiz.mjs` | `quiz` | end of `check`, pass or fail |
| `tools/novakai/verify-change.mjs` | `verdict` | where the closed-form verdict is printed |
| `tools/novakai/plan-cert.mjs` | `verdict` | where the cert verdict is decided |
| `package.json` `novakai:ship` | `ship` | via `metrics.mjs wrap` (section 5) |
...
The current `novakai:ship` chain moves verbatim to a new script `novakai:ship:steps`, and
`novakai:ship` becomes a transparent wrapper:

```jsonc
"novakai:ship":       "node tools/novakai/metrics.mjs wrap --event ship -- npm run novakai:ship:steps",
"novakai:ship:steps": "npm run novakai:bundle && … (today's chain, unchanged)"
```
...
The name `novakai:ship` is preserved deliberately: `ship-staleness.mjs` demands it by name in
...
## 6. Summarizer CLI spec — `tools/novakai/metrics.mjs`

Wired as:

```jsonc
"novakai:metrics": "node tools/novakai/metrics.mjs summary"
```
...
Log path: `docs/novakai/metrics/session-log.jsonl`. The whole `docs/novakai/metrics/` directory
is **gitignored** (the emitter mkdirs it on first write; nothing is committed).

Rationale:

- It is session/machine-local telemetry, exactly like `.novakai-quiz-pass.json` and
...
- **Fail-silent cost, named honestly**: a broken emitter silently undercounts. Accepted,
  ...
  fail-open rule. `NOVAKAI_METRICS_DEBUG=1` is the escape hatch.

## 9. Roadmap predicate conversion

The M2b `manual` note is replaced by machine predicates in `docs/novakai/mvp-roadmap.json`
(this session):

```json
"checks": [
  { "kind": "file", "path": "tools/novakai/metrics.mjs" },
  { "kind": "file", "path": "docs/novakai/m2b-metrics-design.md" },
  { "kind": "grep", "path": "package.json", "pattern": "\"novakai:metrics\":" },
  { "kind": "grep", "path": "tools/novakai/edit-gate.mjs", "pattern": "lib/metrics-log" },
  { "kind": "grep", "path": "tools/novakai/quiz.mjs", "pattern": "lib/metrics-log" },
  { "kind": "grep", "path": "package.json", "pattern": "metrics\\.mjs wrap --event ship" },
  { "kind": "cmd",  "run": "node tools/novakai/metrics.mjs summary --json" }
]
```
...
`npm run novakai:mvp` computes where M2b stands against these checks at any moment — that
command, not this doc, is the status.

## 10. Build plan for the follow-up session (test-first)

1. `tools/novakai/lib/metrics-log.test.mjs` — append shape, fail-silent on unwritable dir,
   `NOVAKAI_ROOT` seam. Then the lib.
2. `tools/novakai/metrics.test.mjs` — summary on: no log, empty log, well-formed log,
...
4. `package.json`: add `novakai:metrics`, split `novakai:ship` / `novakai:ship:steps`,
   register `metrics.test.mjs` (and the lib test) in `spec:test:all`.
5. `.gitignore`: `docs/novakai/metrics/` with the standard reasoned comment.
6. Tooling self-map: `%% src` nodes for `metrics.mjs` and `lib/metrics-log.mjs` in a
   `.novakai.mmd` fragment, regenerate `_tooling.mmd`, `npm run novakai:tooling:verify`.
