...
The pass artifact `.novakai-quiz-pass.json` moves to v2, designed once, landed across
...
- `quiz check` also records `fragments`: sha256 per colocated fragment
  (`src/**/*.novakai.mmd` + `src/main.novakai.mmd`), keyed by the fragment's
...
Each item lands red-then-green in its own commit; the full suite, `novakai:ship`
cleanliness and `novakai:roadmap:audit` are re-proven per commit.
